...
def _get_vertex_model_batch():
  ...
  return {
      'class_label': tf.range(_BATCH_SIZE), # generate vertex model batch --- generate a batch of vertices
...
def _get_face_model_batch():
  ...
  return {
      ...
      'vertices_mask': tf.placeholder(
          dtype=tf.float32, shape=[_BATCH_SIZE, None]), # vertices mask --- mask out invalid vertices in the generation procedure
...
class VertexModelTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.model = VertexModel(
        ...
        quantization_bits=_QUANTIZATION_BITS, # quantization bits --- for vertex prediction
        use_discrete_embeddings=_VERTEX_MODEL_USE_DISCRETE_EMBEDDINGS) # discrete embedding --- for quantized vertex value embedding?
  ...
  def test_sample_outputs_range(self):
    ...
    sample_dict = self.model.sample( # max sample length vertices
...
class FaceModelTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.model = FaceModel(
        encoder_config=_TRANSFORMER_CONFIG, # transformer encoder
        decoder_config=_TRANSFORMER_CONFIG, # transformer decoder
        class_conditional=False, #
        max_seq_length=_NUM_INPUT_FACE_INDICES, # what does `max_seq_length` represent?
        decoder_cross_attention=_FACE_MODEL_DECODER_CROSS_ATTENTION, # face model decoder (pointer network) config
        use_discrete_vertex_embeddings=_FACE_MODEL_DISCRETE_EMBEDDINGS, # use discrete face embeddings
        quantization_bits=_QUANTIZATION_BITS) # quantization bits --- number of bits for quantization

  def test_model_runs(self): # test model runs
    """Tests if the model runs without crashing."""
    batch = _get_face_model_batch()
    pred_dist = self.model(batch, is_training=False)
    logits = pred_dist.logits
    with self.session() as sess:
      sess.run(tf.global_variables_initializer()) # global variables initializer
      vertices = np.random.rand(_BATCH_SIZE, _NUM_INPUT_VERTS, 3) - 0.5 # vertices ----
      vertices_mask = np.ones([_BATCH_SIZE, _NUM_INPUT_VERTS])
      faces = np.random.randint(
          _NUM_INPUT_VERTS + 2, size=[_BATCH_SIZE, _NUM_INPUT_FACE_INDICES]) # max input vertices
      sess.run(
          logits,
  ...
  def test_sample_outputs_range(self): # test sample output ranges
    """Tests if the model produces samples in the correct range."""
    ...
    with self.session() as sess:
      ...
      vertices = np.pad(
          np.random.rand(_BATCH_SIZE, _NUM_INPUT_VERTS, 3) - 0.5, # number input
      ...
      # whether the vertices of faces are in range ---- max_input_verts
      in_range = np.logical_and(
          0 <= sample_dict_np['faces'], # faces... sample dict np # faces
          sample_dict_np['faces'] <= _NUM_INPUT_VERTS + 1).all() # number input vertices
      self.assertTrue(in_range)
