...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, inputs, sequential_context_embeddings=None, mask=None, sequential_context_mask=None):
        ...
        if mask is None:
          ...
          atten_mask = atten_mask.unsqueeze(1).contiguous().repeat(1, self.num_heads, 1, 1).contiguous().view(bsz * self.num_heads, mask.size(-1), mask.size(-1))
...
class VertexModel(nn.Module):
    ...
    def _create_dist(self, batch, embedding_layers, project_to_logits_layer, global_context_embedding=None, sequential_context_embeddings=None, temperature=1., top_k=0, top_p=1.0, rt_logits=False):
        # todo: cache!
        # if vertices.size(1) > 0:
        #     print(f"max of vertices: {torch.max(vertices)}, min of vertices: {torch.min(vertices)}")
        vertices = batch['mask_vertices_flat'] # [:, :-1] # vertices_flat
        ...
        vertices_mask = torch.cat(
          [torch.ones((decoder_inputs.size(0), 1), dtype=torch.float32).cuda(), vertices_mask], dim=1
        )
        
        # decoder_inputs = decoder_inputs * vertices_mask.unsqueeze(-1)
        # 
        outputs = self.decoder(decoder_inputs, mask=vertices_mask)

        outputs = outputs[:, 1:]
        # print(f"after vertices decoding ... outputs: {outputs.size()}")
        ...
        rt_features = torch.softmax(logits, dim=-1) if rt_logits else outputs
        return cat_dist, rt_features
    ...
    def forward(self, batch):
        ...
        pred_dist, outputs = self._create_dist(batch, embedding_layers=self.inputs_embedding_layers, project_to_logits_layer=self.project_to_logits, global_context_embedding=global_context, sequential_context_embeddings=seq_context,
        )
    ...
    def _sample_vertices(self, num_samples, inputs_embedding_layers, project_to_logits, context_feature, seq_context, temperature, top_k, top_p): # sample
        ...
        probs = torch.zeros([num_samples, 0], dtype=torch.float32).cuda()
    ...
    def sample(self, num_samples, context=None, max_sample_length=None, temperature=1., top_k=0, top_p=0.95, recenter_verts=True, only_return_complete=True): # only the largets value is considered?
        ...
        pred_dist, outputs = self._create_dist(context, embedding_layers=self.inputs_embedding_layers, project_to_logits_layer=self.project_to_logits, global_context_embedding=global_context, sequential_context_embeddings=seq_context, rt_logits=True
        )

        vertices = context['vertices_flat']
        # vertices_mask = context['vertices_flat_mask']
        vertices_mask = context['mask_vertices_flat_indicator']
        # sampled_vertices = pred_dist.sample() # bsz x nn_verts_flat
        sampled_vertices = torch.argmax(outputs, dim=-1) # argmax for sampling
        samples = (1. - vertices_mask) * vertices.float() + sampled_vertices.float() * vertices_mask
        samples = samples.long() # samples
        # outputs: bsz x nn_verts x n_discreted_idxes
        probs = data_utils.batched_index_select(values=outputs, indices=sampled_vertices.unsqueeze(-1), dim=2)
