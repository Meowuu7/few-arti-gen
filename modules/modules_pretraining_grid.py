...
class VertexModel(nn.Module):
    ...
    #   grid_coord_embeddings = []
      ...
      grid_content_embedding = grid_content_embedding + grid_embedding
      ...
      # grid order coord xyz embeddings # order_coord_xyz
    ...
    def _create_dist_grid_content(self, grid_embedding, grid_content_embedding, temperature=1., top_k=0, top_p=1.):
      bsz, grid_length = grid_embedding.size(0), grid_embedding.size(1) - 1
...
# 
    ...
    def _create_dist_grid_content_v2(self, grid_embedding, grid_content_embedding, grid_content, is_training=True, temperature=1., top_k=0, top_p=1.):
      ...
      if is_training:
        ...
        # grid value embedding; value encoding + positional encoding --> grid values
    ...
    # 
        grid_values = []
        for i_x in range(self.grid_size ** 3): # grid values 
            # cur_grid_value_proj_layer: bsz x grid_length x 2
        ...
        cur_grid_content_outputs = grid_content_outputs[:, -1:, :] # bsz x 1 x embedding # content outputs # content embedding; grid content encoded values
        # grid_values 
        ...
        for i_g in range(self.grid_size ** 3):
            ...
            # value embedding + positional encoding; jpositional encoding
            ...
            grid_content_feat = flat_grid_content_feat.contiguous() # bsz x (gs ** 3) x embedding_dim # gs ** 3 # flat_grid_content_feat
            ...
            # cur_grid_content_value /= 2.0
            # cur_grid_value...
            # cur_grdi_value: bsz x 2
            cur_grid_content_value = layer_utils.top_k_logits(cur_grid_content_value, top_k) # grid content value
            ...
            cur_grid_content_value = cur_grid_content_value.sample() # bsz # cur_grid_content_value: bsz x 2 
            ##### sampling based #####

            # cur_grid_content_value = torch.argmax(cur_grid_content_value, dim=-1) # bsz x 2 --> bsz
    ...
    def _sample_grids(self, num_samples, context_feature, seq_context, temperature, top_k, top_p):
      ...
      while True:
        ...
        if self._stop_cond_grid(grid_xyzs) or loop_idx >= self.max_num_grids:
          break
        # grid_xyzs: bsz x (1 + 1 + cur_seq_length) x 3
        grid_xyzs = torch.cat(
