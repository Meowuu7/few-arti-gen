...
class TransformerDecoderForPretraining(nn.Module):
    ...
    def forward(self, inputs, sequential_context_embeddings=None, mask=None):
        seq_length = inputs.size(1)
        bsz = inputs.size(0)
...
class VertexModel(nn.Module): # vertex model part?
    ...
    def _create_dist(self, vertices, vertices_mask, embedding_layers, project_to_logits_layer, global_context_embedding=None, sequential_context_embeddings=None, temperature=1., top_k=0, top_p=1.0):
        # todo: cache!
    ...
    def forward(self, batch):
        ...
        pred_dist, outputs = self._create_dist(batch['vertices_flat_masked'], batch['vertices_flat_mask'], embedding_layers=self.inputs_embedding_layers, project_to_logits_layer=self.project_to_logits, global_context_embedding=global_context, sequential_context_embeddings=seq_context)
    ...
    def _sample_vertices(self, num_samples, inputs_embedding_layers, project_to_logits, context_feature, seq_context, top_k, top_p):
        loop_idx = 0
        # init samples
        samples = torch.zeros([num_samples, 0], dtype=torch.long).cuda()
        while True: # _loop_sample for samples
            loop_idx, samples, outputs = self._loop_sample(loop_idx, samples, embedding_layers=inputs_embedding_layers, project_to_logits_layer=project_to_logits, context_feature=context_feature, seq_context=seq_context, top_k=top_k, top_p=top_p)
    ...
    def sample(self, num_samples, context=None, max_sample_length=None, temperature=1., top_k=0, top_p=0.95, recenter_verts=True, only_return_complete=True): # only the largets value is considered?
        ...
        cat_dist, outputs = self._create_dist(
            context['vertices_flat_masked'], context['vertices_flat_mask'], embedding_layers=self.inputs_embedding_layers, project_to_logits_layer=self.project_to_logits, global_context_embedding=global_context, sequential_context_embeddings=seq_context, top_k=top_k, top_p=top_p
        )
        ...
        # vertices_mask_identifier
        samples = context['vertices_flat_masked'].float() * (1. - context['vertices_mask_identifier']) + samples.float() * (context['vertices_mask_identifier'])
        ...
        outputs = { # get vertices and vertices masks
            'completed': completed,  #
            'vertices': vertices,
...
class FaceModel(nn.Module):
    def __init__(self,
        ...
        # class embedding for face conditional setting
        self.class_embedding_layer = nn.Embedding(num_embeddings=self.num_classes, embedding_dim=self.embedding_dim)
    ...
    def _prepare_context(self, context):
        if self.class_conditional: # class_label
            ...
            global_context_embedding = None
        # vertex embeddings; embed vertcies
        vertex_embeddings = self._embed_vertices( # embed 
            context['vertices'],
