...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, inputs, inputs_mask=None): ### inputs, inputs_mask
        ### padding 
        if inputs_mask is None:
            encoder_padding = layer_utils.embedding_to_padding(inputs) # bsz x n_vertices
        else: ### inputs_mask: bsz x nn_vertices
            encoder_padding = inputs_mask # inputs_mask: bsz x nn_vertices #### bsz x seq_length
        bsz = inputs.size(0)
        ...
        # encoder_self_attention_mask: bsz x seq_length; encoding_padding and...
...
class TransformerDecoderGrid(nn.Module):
    ...
    def forward(self, inputs, sequential_context_embeddings=None, mask=None, sequential_context_mask=None, context_window=None, prefix_length=None):
        ...
        context_window = opt.model.context_window if context_window is None else context_window # context window
        seq_length = inputs.size(1)
        # bsz = inputs.size(0) # inputs
        
        # seq_length x seq_length --> attentiion; triangle matrix as the attention mask
        atten_mask = np.tri(seq_length, seq_length, -1.0, dtype=np.float32).T # tri
        # atten_mask = np.tri(seq_length, seq_length, 0.0, dtype=np.float32)
        ##### 
        atten_mask = torch.from_numpy(atten_mask).float().cuda()
        ...
        if context_window > 0: # context window
        ...
        # context attention mask
        atten_mask = (atten_mask > 0.5)
        atten_mask[:, :prefix_length] = False # prefix features should not be masked
        
        if sequential_context_embeddings is not None: # sequential context embeddings
...
class VertexModel(nn.Module):
    def __init__(self,
        ...
        self.nn_max_vertices = self.max_num_grids
        ...
        self.subd_to_grid_embedding_layers = nn.ModuleDict( {
            str(i_subd): nn.ModuleList(
                [
                    ...
                    nn.Sequential(
                        nn.Linear(in_features=1, out_features=self.embedding_dim // 2, bias=True),
                        nn.ReLU(),
                        nn.Linear(in_features=self.embedding_dim // 2, out_features=self.embedding_dim, bias=True),
                        nn.ReLU(),
                        nn.Linear(in_features=self.embedding_dim, out_features=self.embedding_dim, bias=True),
                    ),
                    nn.Embedding(num_embeddings=3, embedding_dim=self.embedding_dim), # xyz indicator embedding
                    # nn.ReLU(),
                    # nn.Embedding(num_embeddings=self.grid_pos_embed_max_num + 5, embedding_dim=self.embedding_dim), # grid order embedding
        ...
        self.project_to_logits = nn.ModuleList(
                [
                    nn.Linear(self.embedding_dim, self.num_grids_quantization + 1, bias=True),
                    nn.Linear(self.embedding_dim, self.num_grids_quantization + 1, bias=True),
                    nn.Linear(self.embedding_dim, self.num_grids_quantization + 1, bias=True),
    ...
    def _embed_pts(self, cur_subd_prefix_pts, embedding_layers):
        # print(f"In embed pts, cur_subd_prefix_pts: {cur_subd_prefix_pts.size()}, nn_order_embeddings: {embedding_layers[0].weight.size()} ")
        ...
        # dequan_cur_subd_prefix_pts_np = data_utils.dequantize_verts_torch(cur_subd_prefix_pts, n_bits=self.quantization_bits)
        cur_subd_upsample_vert_embedding = embedding_layers[1](cur_subd_prefix_pts.unsqueeze(-1)) # bsz x n_verts x 3 x embedding_dim
    ...
    #       print("here...2")
      ...
      # i_subd_for_embedding = range(subdn - 1) if i_subd is None else [i_subd] #### 
      ...
      # cur_subd_upsample = batch[f'subd_{i_subd}_upsample'] # bsz x n_verts xcccccccc 3
      cur_subd_upsample = batch[f'vertices'] # bsz x n_verts x 3 ---> and are continuous values!
      ...
      cur_subd_upsample_vert_embedding = self._embed_pts(cur_subd_upsample, self.subd_to_grid_embedding_layers[str(i_subd)])
      ...
      cur_subd_upsample_vert_embedding = cur_subd_upsample_vert_embedding.cuda()
    ...
    #   prefix_length = cur_subd_prefix_pts_nn + self.prefix_key_len
      ...
      cur_subd_embedding = subd_to_verts_embedding # ### cur_subd_embedding #### 
      # if not self.
      # print(subd_to_verts_embedding.size(), prefix_length)
      ...
      cur_project_to_logits_layers = self.project_to_logits # self.subd_to_grid_project_to_logits[i_subd]
      ...
      mu_xyz = cat_dist_grid_xyz.sample()
      sigma_xyz = torch.ones_like(mu_xyz)
      # #### use categorical distribution ####
      ...
      # )
      # xyz_rnd = torch.randn_like(mu_xyz) # bsz x n_grids x 3
      # delta_xyz = mu_xyz + xyz_rnd * sigma_xyz # * 2.0 # * 0.5 #### sigma_xyz #### sigma_xyz;
      # ### print mu and sigma out for 
      # # sigma_xyz: bsz x n_verts x 3
      # # sigma_reg = torch.sum(sigma_xyz ** 2, dim=-1).mean(-1)
      
      # cat_dist_grid_xyz = delta_xyz  # * 0.5 #### delta_xyz 
      
      # cat_dist_grid_xyz =mu_xyz
      # cat_dist_grid_xyz = torch.distributions.normal.Normal(loc=mu_xyz, scale=0.5)
      ##### store all statistics here #####
      
      subd_to_dist_xyz = {
          'delta_xyz': cat_dist_grid_xyz, 'mu_xyz': mu_xyz, 'sigma_xyz': sigma_xyz
      }
        ...
        for i_subd in range(subdn - 1):
            ...
            if sample_batch[f'subd_{i_subd}_upsample'].size(1) < self.max_num_grids: #### 
                ...
                if isinstance(subd_to_dist_xyz[i_subd], torch.distributions.Categorical):
                    ...
                    cur_subd_upsample_xyz = cur_subd_xyz_dist.sample() # bsz x n_verts x 3
                ...
                # subd_to_dist_xyz = self._create_dist_grid_coord_v2(batch, subd_to_verts_embedding,  temperature=1., top_k=0, top_p=1.0, rt_logits=False)
                        cur_patch_batch = {}
                        for prv_subd in range(i_subd - 1):
            ...
            sample_batch[f'subd_{i_subd + 1}'] = cur_subd_upsample_xyz
    ...
    def _stop_cond(self, samples):
        ...
        # equal_to_zero = (samples < -0.6 or samples > 0.6).long() # equal to zero
        equal_to_zero = (((samples < -0.6).long() + (samples > 0.6).long()) > 0.5).long()
        equal_to_zero = (equal_to_zero.sum(-1) > 0.5).long() # bsz x nn_verts

        accum_equal_zero = torch.sum(equal_to_zero, dim=-1) #### == 0 or others
        
        not_stop = (accum_equal_zero == 0).long() # 
        not_stop = torch.sum(not_stop).item() > 0 # not stop
        return (not not_stop)

    def _stop_cond_quant(self, samples):
        # print(f"in stop condition, samples: {samples}")
        # equal_to_zero = (samples < -0.6 or samples > 0.6).long() # equal to zero
        equal_to_zero = (samples == 0).long()
        equal_to_zero = (equal_to_zero.sum(-1) > 0.5).long() # bsz x nn_verts
    ...
    def _create_sampled_vertices_mask(self, vertices):
      # equal_to_zero = (vertices < -0.6 or vertices > 0.6).long() # equal to zero
      equal_to_zero = (((vertices < -0.6).long() + (vertices > 0.6).long()) > 0.5).long()
      equal_to_zero = (equal_to_zero.sum(-1) > 0.5).float() # bsz x nn_verts #### 1 denotes masked vertices
      vertices_mask = 1.0 - equal_to_zero
      return vertices_mask

    def _create_sampled_vertices_mask_quant(self, vertices):
      # equal_to_zero = (vertices < -0.6 or vertices > 0.6).long() # equal to zero
      # equal_to_zero = (((vertices < -0.6).long() + (vertices > 0.6).long()) > 0.5).long()
      equal_to_zero = (vertices == 0).long()
      equal_to_zero = (equal_to_zero.sum(-1) > 0.5).float() # bsz x nn_verts #### 1 denotes masked vertices
      vertices_mask = 1.0 - equal_to_zero
      return vertices_mask
    ...
    def _sample_vertices(self, num_samples, context, context_feature): # sample
        ...
        vertices = torch.zeros((num_samples, 1, 3), dtype=torch.float32).cuda()
        sample_batch['vertices'] = vertices
        quant_vertices = torch.zeros([num_samples, 1, 3], dtype=torch.long).cuda()
        # sample_batch['vertices_mask'] = 
        ...
        while True: # _loop_sample for samples
            ...
            cur_sample = subd_to_dist_xyz['delta_xyz'] # 
            cur_sample = cur_sample.sample()
            # vertices[:, -1, 0] = cur_sample[:, -1, 0]

            quant_vertices[:, -1, 0] = cur_sample[:, -1, 0]
            sample_batch['vertices'][:, -1, 0] = data_utils.dequantize_verts_torch(cur_sample[:, -1, 0] - 1, n_bits=self.quantization_bits)
            ...
            quant_vertices[:, -1, 1] = cur_sample[:, -1, 1]
            sample_batch['vertices'][:, -1, 1] = data_utils.dequantize_verts_torch(cur_sample[:, -1, 1] - 1, n_bits=self.quantization_bits)
            ...
            quant_vertices[:, -1, 2] = cur_sample[:, -1, 2]
            sample_batch['vertices'][:, -1, 2] = data_utils.dequantize_verts_torch(cur_sample[:, -1, 2] - 1, n_bits=self.quantization_bits)

            # if self._stop_cond(sample_batch['vertices']) or nn_sampled >= self.nn_max_vertices:
            #   # if self._stop_cond(sample_batch['vertices']) :
            #   sample_batch['vertices_mask'] = self._create_sampled_vertices_mask(sample_batch['vertices'])
            #   break
            
            if self._stop_cond_quant(quant_vertices) or nn_sampled >= self.nn_max_vertices:
              # if self._stop_cond(sample_batch['vertices']) :
              sample_batch['vertices_mask'] = self._create_sampled_vertices_mask_quant(quant_vertices)
              break
          
            sample_batch['vertices'] = torch.cat(
              [sample_batch['vertices'],  torch.zeros((num_samples, 1, 3), dtype=torch.float32).cuda()], dim=1
            )
            quant_vertices = torch.cat(
              [quant_vertices, torch.zeros((num_samples, 1, 3), dtype=torch.long).cuda()], dim=1
            )
...
class FaceModel(nn.Module):
    def __init__(self,
        ...
        self.quantization_bits = quantization_bits
        self.max_seq_length = max_seq_length # --max-permit-faces
        self.max_seq_length = opt.dataset.max_permit_faces #  + 1000
        ...
        self.decoder = TransformerDecoder(**decoder_config) #### with seqcontex? 
        ...
        # class embedding for face conditional setting; embedding layers
        ...
        self.vertex_embedding_layer = nn.Sequential(
                        nn.Linear(in_features=3, out_features=self.embedding_dim // 2, bias=True),
                        nn.ReLU(),
                        nn.Linear(in_features=self.embedding_dim // 2, out_features=self.embedding_dim, bias=True),
                        nn.ReLU(),
                        nn.Linear(in_features=self.embedding_dim, out_features=self.embedding_dim, bias=True),
                    )
        # stopping embedding; stopping embedding
        ...
        #### face vertices ####
        self.face_vertices_embedding_layer = nn.Embedding(self.max_seq_length + 1000 + 5, self.embedding_dim)
        #### zero embeedding var ####
    ...
    def _embed_vertices(self, vertices, vertices_mask, vertex_embedding_layer, vertex_stopping_embedding_var):
        ...
        # vertex embeddings
        vertex_embeddings = vertex_embedding_layer(vertices)
        ...
        vertices_mask_for_encoder = 1.0 - vertices_mask_for_encoder
        # print(f"after adding stopping embedding, vertex_embeddings: {vertex_embeddings.size()}")
        
        ##### vertex
        vertex_embeddings = self.encoder(vertex_embeddings, inputs_mask=vertices_mask_for_encoder)
    ...
    def _prepare_context(self, context):
        ...
        if self.decoder_cross_attention:
            # vertex_embedding --- bz x n_seq x dim xxx decorder_cross
    ...
    def _embed_inputs(self, faces_long, vertex_embeddings, face_vertices_embedding_layer, face_vertices_zero_embedding_var, global_context_embedding=None):
        ...
        # print(f"vertex_embeddings: {vertex_embeddings.size()}, faces_long: {faces_long.size()}")
    ...
    def _create_dist(self,
        ...
        """Outputs categorical dist for vertex indices.""" #### vertex indices
        ##### subd meshes #####
        # Embed inputs
    ...
    def sample(self,
        ...
        # 
        max_sample_length = max_sample_length if max_sample_length is not None else self.max_seq_length # or self.max_seq_length
        ...
        #### 
        complete_samples = torch.any((f == 0).float(), axis=-1) # complete samples
        ...
        # num_face_indices --- ### complete sample or the last face....
        ...
        faces_mask = (torch.arange(0, sample_length).unsqueeze(0).cuda() < num_face_indices.unsqueeze(1)).long()
        f *= faces_mask
