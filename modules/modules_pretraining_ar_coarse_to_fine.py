...
# return glboal featuress from sparse points 
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, inputs, sequential_context_embeddings=None, mask=None, sequential_context_mask=None): # forward the inputs for others...
...
class VertexModel(nn.Module): # vertex model part?
    def __init__(self,
        ...
        # class embedding; class conditional embeddings
        ...
        ''' Coarse points embedding layer '''
        # for the fine points embeddings; 
        self.coarse_inputs_embedding_layers = nn.ModuleList([
        ...
        ]) # coarse input embedidng
        for cur_layer in self.coarse_inputs_embedding_layers:
            torch.nn.init.xavier_uniform_(cur_layer.weight)
        ''' Coarse points embedding layer '''

        ''' Fine points embedding layer '''
        # for the fine points embeddings; 
        self.inputs_embedding_layers = nn.ModuleList([
            ...
            nn.Embedding(num_embeddings=self.max_num_input_verts * 2 + 5, embedding_dim=self.embedding_dim), # tf.floordiv(tf.range(seq_length), 3)
            nn.Embedding(num_embeddings=2**self.quantization_bits + 2, embedding_dim=self.embedding_dim), # quantized vertices
            nn.Embedding(num_embeddings=2, embedding_dim=self.embedding_dim)
        ])
    ...
    def _embed_inputs(self, vertices, vertices_identifier, embedding_layers, global_context_embedding=None, add_global_context_embedding=True):
        bz = vertices.size(0)
        ...
        ##### vertices identifier embedding #####
        vertices_identifier_embedding = embedding_layers[3](vertices_identifier)
        vertices_embedding = vertices_embedding + vertices_identifier_embedding
        ##### vertices identifier embedding #####
        ...
        if add_global_context_embedding:
    ...
    def _create_dist(self, vertices, vertices_identifier, global_context_embedding=None, temperature=1., top_k=0, top_p=1.0):

        # coarse_inputs = self._embed_inputs(coarse_vertices, self.coarse_inputs_embedding_layers, global_context_embedding=global_context_embedding, add_global_context_embedding=True)
        # print("HEer2")
        decoder_inputs = self._embed_inputs(vertices, vertices_identifier, self.inputs_embedding_layers, global_context_embedding=global_context_embedding, add_global_context_embedding=True)
        # print("HEer3")

        # decoder_inputs = torch.cat(
        #   [coarse_inputs, fine_inputs], dim=1
        # )
    ...
    def forward(self, batch, return_global_feature=False): # xyz and positional encoding
        ...
        # shold use the pred_dist and vertices_flat_mask!
        # print("HEer1")
        pred_dist, outputs = self._create_dist(batch['vertices_flat'][:, :-1], batch['vertices_identifier'][:, :-1], global_context_embedding=global_context)
    ...
    def _loop_sample(self, loop_idx, samples, samples_identifier, context_feature, top_k, top_p):
        cat_dist, outputs = self._create_dist(
            samples, samples_identifier, global_context_embedding=context_feature, top_k=top_k, top_p=top_p
        )
        next_sample = cat_dist.sample()
        next_sample = next_sample[:, -1:] # next
        samples = torch.cat([samples, next_sample], dim=1)
        samples_identifier = torch.cat([samples_identifier, torch.ones((samples.size(0), 1), dtype=torch.long).cuda()], dim=1)
        # print(f"in vertex looping sampling, samples: {samples.size()}")
        return loop_idx + 1, samples, samples_identifier, outputs
    ...
    def _sample_vertices(self, num_samples, coarse_vertices, coarse_vertices_identifier,  context_feature, top_k, top_p): # sample
        loop_idx = 0
        # init samples
        # samples = torch.zeros([num_samples, 0], dtype=torch.long).cuda()
        samples = coarse_vertices
        samples_identifier = coarse_vertices_identifier
        while True: # _loop_sample for samples
            loop_idx, samples, samples_identifier, outputs = self._loop_sample(loop_idx, samples, samples_identifier, context_feature=context_feature, top_k=top_k, top_p=top_p)
    ...
    def sample(self, num_samples, context=None, max_sample_length=None, temperature=1., top_k=0, top_p=0.95, recenter_verts=True, only_return_complete=True): # only the largets value is considered?
        ...
        # coarse_vertices: bsz (1) x num_vertices
        coarse_vertices = context['coarse_vertices_flat']
        num_coarse_vertices = context['num_coarse_vertices']
        coarse_vertices = coarse_vertices[:, :num_coarse_vertices[0].item()]
        coarse_vertices_identifier = torch.zeros_like(coarse_vertices).long() # zeros
        ...
        # sample vertice; 
        loop_idx, samples, outputs = self._sample_vertices(num_samples, coarse_vertices, coarse_vertices_identifier,  context_feature=global_context, top_k=top_k, top_p=top_p)

        samples = samples[:, num_coarse_vertices[0].item():]
...
class FaceModel(nn.Module):
    ...
    def _create_dist(self,
                     ...
                     is_training=False,
                     cache=None,
                     rt_decoder_output=False): # and we should use other strategies for finetuning...
    ...
    def forward(self, batch, rt_global_features=False):
