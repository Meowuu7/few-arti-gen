...
class TransformerEncoder(nn.Module):
    def __init__(self,
        ...
        self.with_activaton = with_activaton
        ...
        for i in range(self.num_layers):
            ...
            if self.re_zero: # re_zero_var; ReZero? ReZero?
    ...
    def forward(self, inputs, inputs_mask=None, with_mask=True):
        ### padding 
...
class TransformerDecoderGrid(nn.Module):
    ...
    def forward(self, inputs, sequential_context_embeddings=None, mask=None, sequential_context_mask=None):
        ...
        if opt.model.shift_context_window:
            cur_context_window = np.random.randint(low=1, high=8, size=(1,)).item() * 100
            self.context_window = cur_context_window

        # # context_window; 
        if self.context_window > 0: # context window
            # ##### add global context embeddings to embedding vectors ##### #
            # inputs = inputs[:, 0:1] + inputs # add the contextual information to inputs # not add...
            # if opt.model.debug:
            #     print(f"Using context window {self.context_window} for decoding...")
        ...
        #   atten_mask[:, 0] = False # aware of global context
...
class VertexModel(nn.Module):
    def __init__(self,
        ...
        self.grid_pos_embed_max_num_nn = self.grid_pos_embed_max_num - 1

        self.prefix_key_len = opt.model.prefix_key_len
        self.prefix_value_len = opt.model.prefix_value_len
        self.with_prompt_enocder = with_prompt_enocder
        ...
        if self.grid_size > 1:
            ...
            if use_multi_gpu or not use_multi_gpu:
            # if use_multi_gpu:
                self.grid_embedding_layers = nn.ModuleList(
                [
                    # nn.Embedding(num_embeddings=3, embedding_dim=self.embedding_dim),
        ...
        if self.class_conditional:
            ...
            # # grid content class embedding layers
            # self.class_embedding_layer_content = nn.Embedding(num_embeddings=self.num_classes, embedding_dim=self.embedding_dim)
            # self.class_embedding_layer_grid = nn.Embedding(num_embeddings=self.num_classes, embedding_dim=self.embedding_dim)
            # torch.nn.init.xavier_uniform_(self.class_embedding_layer_content.weight)
            # torch.nn.init.xavier_uniform_(self.class_embedding_layer_grid.weight)

            # with_prompt_enocder
            # 
            if self.with_prompt_enocder:
                class_prompt_attention_config = clone_dict(encoder_config) # clone decoder config for prompt attention config
                #  num_heads=4,
                #  num_layers=8,
                class_prompt_attention_config['num_heads'] = 4
                class_prompt_attention_config['num_layers'] = 3
                print(f"Setting up prompt encoder with config: {class_prompt_attention_config}.")
                ###### Get class prompt encoder mdoule ######
                self.class_prompt_encoder_module = TransformerEncoder(**class_prompt_attention_config)
            ...
            if self.grid_size > 1 or not use_multi_gpu:
            # if self.grid_size > 1:
                ...
                if self.with_prompt_enocder:
                    print(f"Setting up content/grid prompt encoder with config: {class_prompt_attention_config}...")
                    self.content_prompt_encoder_module = TransformerEncoder(**class_prompt_attention_config)
                    self.grid_prompt_encoder_module = TransformerEncoder(**class_prompt_attention_config)
            # torch.nn.init.xavier_uniform_(self.class_embedding_prompts.weight)
            # torch.nn.init.xavier_uniform_(self.class_embedding_layer_content.weight)
            # torch.nn.init.xavier_uniform_(self.class_embedding_layer_grid.weight)
        ...
        if not self.class_conditional: # class condition
    ...
    # #### prepare contex ####
    def _prepare_context(self, context, adapter_modules=None):
        ...
        if self.class_conditional:
            ...
            if adapter_modules is None:
                # print("getting classs conditional prefix embeddings with class_label:", context['class_label'])
                ...
                if self.with_prompt_enocder:
                    global_context_embedding = self.class_prompt_encoder_module(global_context_embedding, with_mask=False)

                if self.grid_size > 1:
                    global_context_embedding_content = self.class_embedding_layer_content[context['class_label']]
                    global_context_embedding_content = global_context_embedding_content.squeeze(1)
                    if self.with_prompt_enocder:
                        global_context_embedding_content = self.content_prompt_encoder_module(global_context_embedding_content, with_mask=False)

                else:
                    global_context_embedding_content = None
                ...
                if self.grid_size > 1:
                    ...
                    if self.with_prompt_enocder:
                        ##### global context embedding grid #####
                        global_context_embedding_grid = self.grid_prompt_encoder_module(global_context_embedding_grid, with_mask=False)
                else:
                    global_context_embedding_grid = None
                ...
                nn_training_categories = opt.model.nn_training_categories
                source_context_embedding = self.class_embedding_layer[:nn_training_categories].data
                source_context_embedding_content = self.class_embedding_layer_content[:nn_training_categories].data
                source_context_embedding_grid = self.class_embedding_layer_grid[:nn_training_categories].data # nn_training_categories x prefix_length x embedding_dim

                # ### source context
                global_context_embedding, global_context_embedding_content, global_context_embedding_grid = adapter_modules([source_context_embedding, source_context_embedding_content, source_context_embedding_grid])

                global_context_embedding = global_context_embedding.unsqueeze(0).contiguous().repeat(bsz, 1, 1)
                global_context_embedding_content = global_context_embedding_content.unsqueeze(0).contiguous().repeat(bsz, 1, 1)
                global_context_embedding_grid = global_context_embedding_grid.unsqueeze(0).contiguous().repeat(bsz, 1, 1)
    ...
    def _embed_input_grids(self, grid_xyzs, grid_content, grid_pos=None, global_context_embedding=None):
      ...
      if grid_pos is None:
        ...
        grid_pos = grid_order.unsqueeze(0).contiguous().repeat(bsz, 1).contiguous()
        ...
        grid_order_embedding = self.grid_embedding_layers[0](grid_pos) # .unsqueeze(0) # grid order, grid pos
      # 
    ...
    #   grid_coord_embeddings = []
      ...
      # grid_embedding: bsz x (1 + grid_length) x embedding_dim
      ...
      if self.grid_size > 1:
        ...
        grid_content_embedding = flat_grid_content_embedding.contiguous().view(bsz, grid_length, self.embedding_dim).contiguous() # grid length
        grid_content_embedding = grid_content_embedding + grid_embedding # content (dummy embeddings), grid coordinates embeddings, grid order embeddings
        # grid context embedding
        # print(f"global_context_embedding: {global_context_embedding.size()}, grid_content_embedding: {grid_content_embedding.size()}")
        grid_content_embedding = torch.cat([global_context_embedding_content, grid_content_embedding], dim=1) # content # content embedding
      else:
        grid_content_embedding = None
      ...
      # grid xyz embedding
        ...
        #### minn order ? #####
      grid_minn_order, _ = torch.min(grid_pos, dim=-1) # grid_minn_order: bsz
      grid_minn_order = grid_minn_order - 1
      grid_minn_order[grid_minn_order < -0.1] = self.grid_pos_embed_max_num_nn
      grid_minn_order_embeddings = self.grid_embedding_layers[0](grid_minn_order) # bsz x embedding_dim
      # #### global_context_embedding: bsz x 1 x embedding_dim #### # glboal_minn_order_embeddings
      global_context_embedding = grid_minn_order_embeddings.unsqueeze(1) + global_context_embedding # bsz x 1 x embedding_dim
        #### minn order ? #####
      
      # grid_order_coord_xyz_embeddings: bsz x grid_length x 3 x embedding_dim
      # xyz embeddings; grid_xyz_embedding
      ...
      grid_order_coord_xyz_embeddings = grid_order_coord_xyz_embeddings.contiguous().view(bsz, grid_length * 3, -1).contiguous() # flat order_coord_xyz_embeddings
      # grid order coord xyz embeddings
      # for order/coordinate query/key/value; 
      ...
      return grid_embedding, grid_content_embedding, grid_order_coord_xyz_embeddings

    # def _create_dist_grid_coord(self, grid_order_embedding, grid_content_embedding,  temperature=1., top_k=0, top_p=1.0):
    #   # bsz, grid_length = grid_order_embedding.size(0), grid_order_embedding.size(1) - 1
    #   # grid_coord_outputs = self.decoder_grid_coord(grid_order_embedding, sequential_context_embeddings=grid_content_embedding)
    #   grid_coord_outputs = self.decoder_grid_coord(grid_content_embedding, sequential_context_embeddings=grid_order_embedding)

    #   # grid_coord_outputs = grid_coord_outputs[:, 1:, :]

    #   logits_x, logits_y, logits_z = self.grid_project_to_logits[0](grid_coord_outputs), self.grid_project_to_logits[1](grid_coord_outputs), self.grid_project_to_logits[2](grid_coord_outputs)
    ...
    ##### creat dist for grid coordiantes #####
    ...
    #   bsz, grid_length = grid_order_embedding.size(0), grid_order_coord_xyz_embeddings.size(1) - 1
      # grid_coord_outputs = self.decoder_grid_coord(grid_order_embedding, sequential_context_embeddings=grid_content_embedding)
      # grid_coord_outputs-
      # grid coord embeddings; grid_order_coord_xyz_embedding
      # grid_order_coord_xyz_embeddings: coord xyz embeddings # grid_order_coord_xyz_embeddings
      grid_coord_outputs = self.decoder_grid_coord(grid_order_coord_xyz_embeddings, sequential_context_embeddings=None) # only use coord for coord decoding
    ...
    ##### directly regress from features, treate grids independently #####
    def _create_dist_grid_content(self, grid_embedding, grid_content_embedding, temperature=1., top_k=0, top_p=1.):
      ...
      grid_content_outputs = self.decoder_grid_content(grid_content_embedding, sequential_context_embeddings=grid_embedding)
...
# 
      grid_values = []
      for i_x in range(self.grid_size ** 3):
        # cur_grid_value_proj_layer: bsz x grid_length x 2
        cur_grid_value = self.grid_content_project_to_logits[i_x](grid_content_outputs)
    ...
    def _create_dist_grid_content_v2(self, grid_embedding, grid_content_embedding, grid_content, is_training=True, temperature=1., top_k=0, top_p=1.):
      bsz, grid_length = grid_embedding.size(0), grid_embedding.size(1) - 1 # should have grid_content embedding
      # grid_content: bsz x grid_length x grid_size x grid_size x grid_size
      # grid_content_outputs: bsz x (1 + grid_length) x embedding_dim
      # grid_content_outputs = self.decoder_grid_content(grid_embedding, sequential_context_embeddings=grid_content_embedding)
      # grid_content_outputs: bsz x grid_length x embedding_dim; 
      grid_content_outputs = self.decoder_grid_content(grid_content_embedding, sequential_context_embeddings=grid_embedding) # content embedding
      
      if is_training:
        grid_value_embedding = self.grid_content_embedding_layer(grid_content) # bsz x grid_length x gs x gs x gs x embedding_dim
        
        ##### Add grid positional encoding & decoder for grid content #####
        grid_positional_encoding = torch.arange(start=0, end=self.grid_size ** 3, step=1, dtype=torch.long).cuda()
        grid_positional_encoding = self.grid_positional_encoding(grid_positional_encoding)

        # grid positional encoding + value embedding
        grid_positional_encoding = grid_value_embedding.contiguous().view(bsz * grid_length, self.grid_size ** 3, self.embedding_dim).contiguous() + grid_positional_encoding.unsqueeze(0) # .unsqueeze(0) # bsz x grid_length x (gs x gs x gs) x embedding_dim
        
        flat_grid_content_outputs_for_decoding = torch.cat(
            [grid_content_outputs.contiguous().view(bsz * grid_length, 1, -1).contiguous(), grid_positional_encoding], dim=1
        )
        flat_grid_content_feat = self.decoder_inner_grid_content(flat_grid_content_outputs_for_decoding) # bsz x grid_length x (len) x embedding_dim
        grid_content_feat = flat_grid_content_feat.contiguous().view(bsz, grid_length, -1, self.embedding_dim).contiguous()
        ##### Add grid positional encoding & decoder for grid content #####
    ...
    # 
        ...
        grid_values = torch.cat(grid_values, dim=-2) # 
        # grid_values: bsz x grid_length x grid_size x grid_size x grid_size x 2
        grid_values = grid_values.contiguous().view(bsz, grid_length, self.grid_size, self.grid_size, self.grid_size, 2).contiguous() # sample grid values
        cat_dist_grid_values = torch.distributions.Categorical(logits=grid_values)
      else:
        cur_grid_content_outputs = grid_content_outputs[:, -1:, :] # bsz x 1 x embedding # content outputs
        grid_value = torch.zeros((bsz, self.grid_size, self.grid_size, self.grid_size), dtype=torch.long).cuda()
        
        grid_positional_encoding = torch.arange(start=0, end=self.grid_size ** 3, step=1, dtype=torch.long).cuda()
        grid_positional_encoding = self.grid_positional_encoding(grid_positional_encoding) # positional embedding
        grid_positional_encoding = grid_positional_encoding.unsqueeze(0) 

        for i_g in range(self.grid_size ** 3):
            grid_value_embedding = self.grid_content_embedding_layer(grid_value)
            # value embedding + positional encoding
            grid_positional_encoding = grid_value_embedding.contiguous().view(bsz, self.grid_size ** 3, -1).contiguous() + grid_positional_encoding # .unsqueeze(0) # bsz x (gs x gs x gs) x embedding_dim # 
            
            # print(f"cur_grid_content_outputs: {cur_grid_content_outputs.size()}, grid_positional_encoding: {grid_positional_encoding.size()}")
            flat_grid_content_outputs_for_decoding = torch.cat(
                [cur_grid_content_outputs.contiguous().view(bsz, 1, -1).contiguous(), grid_positional_encoding], dim=1
            )
            flat_grid_content_feat = self.decoder_inner_grid_content(flat_grid_content_outputs_for_decoding) # bsz x (len) x embedding_dim
            grid_content_feat = flat_grid_content_feat.contiguous() # bsz x (gs ** 3) x embedding_dim
            cur_grid_content_value = grid_content_feat[:, i_g] # bsz x embedding_dim
            cur_grid_content_value = self.grid_content_project_to_logits[i_g](cur_grid_content_value) # bsz x embedding_dim / nn_logits
            cur_grid_content_value /= temperature
            # cur_grid_value...
            # cur_grdi_value: bsz x 2
            cur_grid_content_value = layer_utils.top_k_logits(cur_grid_content_value, top_k)
            cur_grid_content_value = layer_utils.top_p_logits(cur_grid_content_value, top_p)
            cur_grid_content_value = torch.distributions.Categorical(logits=cur_grid_content_value)
            cur_grid_content_value = cur_grid_content_value.sample() # bsz
            i_x, i_y, i_z = i_g // (self.grid_size ** 2), ((i_g % (self.grid_size ** 2)) // self.grid_size), i_g % self.grid_size
            grid_value[:, i_x, i_y, i_z] = cur_grid_content_value


        # grid_positional_encoding = grid_value_embedding.contiguous().view(bsz, self.grid_size ** 3, self.embedding_dim).contiguous() + grid_positional_encoding # bsz x grid_length x (gs x gs x gs) x embedding_dim
    ...
    def forward(self, batch, adapter_modules=None):
        global_context, seq_context = self._prepare_context(batch, adapter_modules=adapter_modules) # global_context
        ...
        grid_content = batch['grid_content']
        ...
        grid_embedding, grid_content_embedding, grid_order_coord_xyz_embeddings = self._embed_input_grids(grid_xyzs=grid_xyzs,  grid_content=grid_content, grid_pos=grid_pos, global_context_embedding=global_context)
    ...
    def _loop_sample_grid(self, loop_idx, samples, global_context_embedding=None, sequential_context_embeddings=None, temperature=1., top_k=0, top_p=1.0):
      grid_xyzs = samples['grid_xyzs'] # bsz x (1 + cur_seq_length) x 3
      grid_content = samples['grid_content'] # bsz x (1 + cur_seq_length) x (gs x gs x gs)
      ...
      if nn_grids > max_num_grids:
        grid_xyzs_in = grid_xyzs[:, nn_grids - max_num_grids + 1: ]
        grid_content_in = grid_content[:, nn_grids - max_num_grids + 1: ]
        grid_pos_in = grid_pos[:, nn_grids - max_num_grids + 1: ]
      else:
      ...
      ##### sample x #####
      grid_embedding, grid_content_embedding, grid_order_coord_xyz_embeddings = self._embed_input_grids(grid_xyzs=grid_xyzs_in,  grid_content=grid_content_in,  grid_pos=grid_pos_in, global_context_embedding=global_context_embedding)
    #   grid_order_embedding, grid_embedding, grid_content_embedding, grid_order_coord_xyz_embeddings = self._embed_input_grids(grid_xyzs=grid_xyzs_in,  grid_content=grid_content_in, global_context_embedding=global_context_embedding)
      ...
      grid_xyzs_in[:, -1, 0] = pred_grid_x # grids... # grids
      ##### sample x #####
    ...
    #   grid_order_embedding, grid_embedding, grid_content_embedding, grid_order_coord_xyz_embeddings = self._embed_input_grids(grid_xyzs=grid_xyzs,  grid_content=grid_content, global_context_embedding=global_context_embedding)
      ...
      ####### grid content v2 for sampling 3###### --> a sequential sampling
      # pred_grid_values = self._create_dist_grid_content_v2(grid_embedding, grid_content_embedding, grid_content, is_training=False)
      ...
      samples['grid_content'] = grid_content
    ...
    # sample grids...
    def _sample_grids(self, num_samples, context_feature, seq_context, temperature, top_k, top_p):
      loop_idx = 0
      # grid_xyzs and grid_content
      grid_xyzs = torch.zeros([num_samples, 1, 3], dtype=torch.long).cuda()
      # grid_content; grid_pos;
      ...
      while True:
        ...
        # 
        grid_xyzs = samples['grid_xyzs']; grid_content = samples['grid_content']; grid_pos = samples['grid_pos']

        # if self._stop_cond_grid(grid_xyzs) or loop_idx >= self.max_num_grids: # max_num_grids...
        if self._stop_cond_grid(grid_xyzs) or loop_idx >= 20000:
          break
        # grid_xyzs: bsz x (1 + 1 + cur_seq_length) x 3 # (1 + 1 + cur_seq_length)
        grid_xyzs = torch.cat(
          [grid_xyzs, torch.zeros((num_samples, 1, 3), dtype=torch.long).cuda()], dim=1
        )
        # grid_content: bsz x (1 + 1 + cur_seq_length) x (gs x gs x gs)
        grid_content = torch.cat(
          [grid_content, torch.zeros((num_samples, 1, self.grid_size, self.grid_size, self.grid_size), dtype=torch.long).cuda()], dim=1
        )
        grid_pos = torch.cat(
        ...
        samples['grid_content'] = grid_content
        samples['grid_pos'] = grid_pos
      samples['grid_xyzs'] = grid_xyzs # grid_xyzs
      samples['grid_content'] = grid_content
      samples['grid_pos'] = grid_pos
    ...
    def sample(self, num_samples, context=None, max_sample_length=None, temperature=1., top_k=0, top_p=0.95, recenter_verts=True, only_return_complete=True, adapter_modules=None): # only the largets value is considered?
        # sample
        global_context, seq_context = self._prepare_context(context, adapter_modules=adapter_modules)
        # global_contex # 
        ...
        loop_idx, samples = self._sample_grids(num_samples, global_context, seq_context, temperature, top_k, top_p)
        grid_xyzs = samples['grid_xyzs'] - 1
        grid_values = samples['grid_content']
