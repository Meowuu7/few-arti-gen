...
class VertexModel(nn.Module):
    def __init__(self,
        ...
        if self.class_conditional:
            ...
            self.class_embedding_layer = nn.Parameter( ### prefix_key_len
    ...
    def _prepare_context(self, context):
        if self.class_conditional:
            if isinstance(self.class_embedding_layer, nn.Embedding):
    ...
    def _create_dist(self, vertices, embedding_layers, project_to_logits_layer, global_context_embedding=None, sequential_context_embeddings=None, temperature=1., top_k=0, top_p=1.0, rt_logits=False):
        # todo: cache!
...
class FaceModel(nn.Module):
    def __init__(self,
        ...
        self.face_vertices_zero_embedding_var = nn.Parameter(torch.zeros((1, self.prefix_key_len, self.embedding_dim), dtype=torch.float32, requires_grad=True), requires_grad=True)
    ...
    def _embed_class_label(self, labels):
        if isinstance(self.class_embedding_layer, nn.Embedding):
            ...
            class_embedding = self.class_embedding_layer(labels)
            class_embedding = class_embedding.squeeze(1)
