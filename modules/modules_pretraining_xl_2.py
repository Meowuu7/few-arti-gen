...
class TransformerDecoderForPretraining(nn.Module):
    ...
    # inputs, inputs_pos
    def forward(self, inputs, inputs_pos, sequential_context_embeddings=None, mask=None):
        seq_length = inputs.size(1)
        bsz = inputs.size(0)
...
class VertexModel(nn.Module): # vertex model part?
    ...
    def _embed_inputs(self, vertices, embedding_layers, global_context_embedding=None, vertices_pos=None):
        bz = vertices.size(0)
        ...
        if vertices_pos is None:
          ...
          vertices_pos_embedding = embedding_layers[1](vertices_pos)
          vertices_embedding = (vertices_coords_embedding).unsqueeze(0) + vertices_pos_embedding + vertices_embedding
          vertices_embedding_pos = (vertices_coords_embedding).unsqueeze(0).repeat(bz, 1, 1) + vertices_pos_embedding
    ...
    # create dist
    def _create_dist(self, vertices, vertices_mask, embedding_layers, project_to_logits_layer, global_context_embedding=None, sequential_context_embeddings=None, vertices_pos=None, temperature=1., top_k=0, top_p=1.0, is_training=False):
        # todo: cache!
        decoder_inputs, decoder_inputs_pos = self._embed_inputs(vertices, embedding_layers, global_context_embedding=global_context_embedding, vertices_pos=vertices_pos)
        ...
        outputs = self.decoder(decoder_inputs, decoder_inputs_pos, )

        if is_training:
          outputs = outputs[:, :-1] # for logits to predict vertices
    ...
    def forward(self, batch):
        ...
        pred_dist, outputs = self._create_dist(batch['vertices_flat'], batch['vertices_flat_mask'], embedding_layers=self.inputs_embedding_layers, project_to_logits_layer=self.project_to_logits, global_context_embedding=global_context, sequential_context_embeddings=seq_context, vertices_pos=batch['vertices_positional_orders'], is_training=True)
        # return to predi
    ...
    def _sample_vertices(self, vertices_flat, vertices_flat_mask, context_feature, seq_context, vertices_pos, top_k, top_p):
        loop_idx = 0
        # init samples
        bsz = vertices_flat.size(0)

        samples = vertices_flat[:, :6] # the first 6 vertices points for sampling
        samples_pos = vertices_pos[:, :6]
        # nn_samples_vertices 
        # 
        while True:
          cur_dist, outputs = self._create_dist(
            samples, vertices_flat_mask, embedding_layers=self.inputs_embedding_layers, project_to_logits_layer=self.project_to_logits, global_context_embedding=context_feature, sequential_context_embeddings=seq_context, vertices_pos=samples_pos
          )
          cur_samples = cur_dist.sample()
          cur_samples = cur_samples[:, -1:]
          samples = torch.cat([samples, cur_samples], dim=-1) # (n_bsz x cur_n_samples)
          nn_verts = samples.size(-1)
          verts_pos = (nn_verts - 1) // 3 - 1
          samples_pos = torch.cat([samples_pos, torch.full((bsz, 1), fill_value=verts_pos, dtype=torch.long).cuda()], dim=-1)
    ...
    def sample(self, num_samples, context=None, max_sample_length=None, temperature=1., top_k=0, top_p=0.95, recenter_verts=True, only_return_complete=True): # only the largets value is considered?
        ...
        loop_idx, samples, outputs = self._sample_vertices(
            ...
            context['vertices_flat'],  # use masked vertices for sampling
            context['vertices_flat_mask'], context_feature=global_context, seq_context=seq_context, vertices_pos=context['vertices_positional_orders'], top_k=top_k, top_p=top_p
        )
        ...
        ###### we then have the current samples now #####
        ...
        if recenter_verts: # recenter verts
