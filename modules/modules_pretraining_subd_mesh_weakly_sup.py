...
import utils.operation_utils as operation_utils
...
class VertexModel(nn.Module):
    ...
    def _create_dist_loop_subdiv(self, batch,  temperature=1., top_k=0, top_p=1.0, rt_logits=False, global_context_embedding=None):
        ...
        for i_subd in range(subdn - 1): ###### subdn_to_embeddings
            ...
            # cur_subd_edges = batch[f'subd_{i_subd}_edges'] # bsz x 2 x nn_edges_verts 
            nex_subd_edges = batch[f'subd_{i_subd}_upsample_edges']
            cur_subd_fake_upsample_faces = batch[f'subd_{i_subd}_fake_upsample_faces']
            
            if f'subd_{i_subd}' in sampled_subd_to_pts:
                cur_subd = sampled_subd_to_pts[f'subd_{i_subd}']
            else:
                cur_subd = batch[f'subd_{i_subd}']
            cur_subd_nn_pts = cur_subd.size(1)
            if i_subd == 0:
                cur_subd_fake_faces = batch[f'subd_{i_subd}_faces']
            else:
                cur_subd_fake_faces = batch[f'subd_{i_subd - 1}_fake_upsample_faces']
            # print(f"cur_subd: {cur_subd.size()}, cur_subd_fake_faces: {cur_subd_fake_faces.size()},  cur_subd_upsample: {cur_subd_upsample.size()}, max_cur_subd_fake_faces: {torch.max(cur_subd_fake_faces)}, min_cur_subd_fake_faces: {torch.min(cur_subd_fake_faces)}")
            #### upsample_verts_cur_subd_weights: bsz x nn_upsample_verts x nn_verts
            upsample_verts_cur_subd_weights = operation_utils.mean_value_coordinates_3D(query=cur_subd_upsample, vertices=cur_subd, faces=cur_subd_fake_faces - 1)
            odd_to_even_weights = upsample_verts_cur_subd_weights[:, cur_subd_nn_pts: ]
            upsample_verts_cur_subd_weights = odd_to_even_weights
            # print("weights", upsample_verts_cur_subd_weights)
            ...
            if opt.model.pred_prob:
                ...
                subd_to_dist_xyz[i_subd] = cat_dist_grid_xyz
                if opt.model.pred_delta:
                  cur_res_pred = cur_subd_upsample + cat_dist_grid_xyz
                else:
                  cur_res_pred = cat_dist_grid_xyz
                  
                ### interpolate using previous subdivison mesh ###
                cur_subd_res_pred = cur_res_pred[:, :cur_subd_nn_pts] #### bsz x nn_verts_0 x 3

                ### bsz x nn_nex_odd x nn_cur xxxxxxxx bsz x nn_cur x 3
                odd_res_pred = torch.matmul(upsample_verts_cur_subd_weights, cur_subd_res_pred) #### bsz x nn_nex x 3
                # print(f"i_subd: {i_subd}, cur_subd_res_pred: {cur_subd_res_pred.size()}, odd_res_pred: {odd_res_pred.size()}")
                cur_res_pred = torch.cat(
                    [cur_subd_res_pred, odd_res_pred], dim=1
                )
                ##### delta predictions #####
                cat_dist_grid_xyz = cur_res_pred - cur_subd_upsample
                if opt.model.pred_delta:
                    subd_to_dist_xyz[i_subd] = cat_dist_grid_xyz
                else:
                    subd_to_dist_xyz[i_subd] = cur_res_pred
            
            subd_to_res_pred[i_subd] = cur_res_pred
            sampled_subd_to_faces[i_subd] = cur_subd_fake_upsample_faces
            ...
            if i_subd < subdn - 2:
                ...
                sampled_subd_to_pts[f'subd_{i_subd + 1}'] = cur_subd_pred_upsample ### current subd upsampled vertices as the next subd's base vertices
                #### nex subdiv faces ####
        ...
        # print(f"after loop subdiv with subd_to_dist_xyz: {subd_to_dist_xyz.keys()}, subd_to_res_pred: {subd_to_res_pred.keys()}, sampled_subd_to_faces: {sampled_subd_to_faces.keys()}") 

            # subd_to_verts_embedding[i_subd] = cur_subd_upsample_vert_embedding.cuda()
        return subd_to_dist_xyz, subd_to_res_pred, sampled_subd_to_faces
    ...
    def sample_forward(self, batch, adapter_modules=None):
        ...
        for i_subd in subd_idxes:
            ...
            # nex_subd_faces = batch[f'subd_{i_subd + 1}_faces']
            nex_subd_faces = batch[f'subd_{i_subd}_fake_upsample_faces'] ### faces for the fake upsample ###s
    ...
    def calculate_mesh_laplacian(self, vertices, edges, vertices_dof):
      lap_ori_mesh = torch.zeros_like(vertices) # bsz x nn_verts x 3
      lap_ori_mesh[:, edges[:, 0] - 1] += vertices[:, edges[:, 1] - 1]
      # print(f"mesh laplacian with lap_ori_mesh: {lap_ori_mesh.size()}, vertices_dof: {vertices_dof.size()}")
      lap_ori_mesh = lap_ori_mesh / torch.clamp(vertices_dof.float().unsqueeze(-1), min=1e-5)
      lap_ori_mesh = vertices - lap_ori_mesh
      return lap_ori_mesh

    #### differential mesh sampling ####
    #### vertices vertices vertices #### sample mesh surface points ####

    def get_mesh_distance_loss(self, subd_to_dist_xyz, sampled_subd_to_faces, gt_faces, ori_upsample_vertices, ori_upsample_edges, ori_dof_vertices, gt_vertices, single_dir_chamfer=True):

        ### subd_to_faces?
      ### vertices based loss ###
      # gt_vertices: bsz x nn_verts x 3
      # ori_dof_vertices: bsz x nn_verts
      #### vertex-based chamfer loss and laplacian loss ####
    
      tot_pts_cd = 0
      bsz = gt_vertices.size(0)
      ###### points based loss ######
    #   for i_bsz in range(bsz):
    #     n_points = 1024
    #     cur_gt_vertices = gt_vertices[i_bsz]
    #     cur_gt_faces = gt_faces[i_bsz]
    #     with torch.no_grad():
    #         gt_shape_sampled_pts = model_utils.sample_pts_from_mesh(cur_gt_vertices, cur_gt_faces, npoints=n_points)
    #     for cur_subd in subd_to_dist_xyz:
    #         cur_subd_verts = subd_to_dist_xyz[cur_subd][i_bsz]
    #         cur_subd_faces = sampled_subd_to_faces[cur_subd][i_bsz]
    #         cur_subd_sampled_pts = model_utils.sample_pts_from_mesh(cur_subd_verts, cur_subd_faces, npoints=n_points)
    #         #### gt_shape_sampled_pts: n_gt_pts x 3; cur_subd_sampled_pts: n_sampled_pts x 3
    #         dist_cur_subd_to_gt = torch.sum((cur_subd_sampled_pts.unsqueeze(1) - gt_shape_sampled_pts.unsqueeze(0)) ** 2, dim=-1) ### n_sampled x n_gt
    #         dist_cur_subd_to_gt, dist_gt_to_cur_subd = torch.min(dist_cur_subd_to_gt, dim=-1)[0], torch.min(dist_cur_subd_to_gt, dim=0)[0]
    #         dist_cur_subd_to_gt = torch.mean(dist_cur_subd_to_gt)
    #         dist_gt_to_cur_subd = torch.mean(dist_gt_to_cur_subd)
    #         tot_pts_cd += dist_cur_subd_to_gt + dist_gt_to_cur_subd
    #   tot_pts_cd /= float(bsz)
    ###### points based loss ######
    
      tot_dist_cur_subd_to_gt_verts = 0.
      last_subd_idx = 0
      last_subd_verts = None
      for cur_subd in subd_to_dist_xyz:
        cur_subd_verts = subd_to_dist_xyz[cur_subd]
        cur_subd_faces = sampled_subd_to_faces[cur_subd]
        
        # cur_subd_verts: bsz x nn_verts_1 x 3
        # gt_vertices: bsz x nn_verts_2 x 3
        dist_cur_subd_verts_gt_verts = torch.sum((cur_subd_verts.unsqueeze(2) - gt_vertices.unsqueeze(1)) ** 2, dim=-1) # bsz x nn_verts_1 x nn_verts_2 --> bsz x nn_verts_1
        dist_cur_subd_verts_to_gt_verts, _ = torch.min(dist_cur_subd_verts_gt_verts, dim=-1) # minn_distance
        dist_cur_subd_verts_to_gt_verts = torch.mean(dist_cur_subd_verts_to_gt_verts, dim=-1) # bsz # mean of the loss over all verts in cur_subd
        tot_dist_cur_subd_to_gt_verts += dist_cur_subd_verts_to_gt_verts ### bsz

        if not single_dir_chamfer:
          dist_gt_verts_to_cur_subd, _ = torch.min(dist_cur_subd_verts_gt_verts, dim=-2)
          dist_gt_verts_to_cur_subd = torch.mean(dist_gt_verts_to_cur_subd, dim=-1) ### dist_gt_verts_to_cur_subd
          #### dist_gt_verts_to_cur_subd: dist of gt_verts to cur_subd verts ####
          tot_dist_cur_subd_to_gt_verts += dist_gt_verts_to_cur_subd
        ### tot_dist_cur_subd_to_gt_verts 

        #### 

        if cur_subd >= last_subd_idx:
          last_subd_idx = cur_subd
          last_subd_verts = cur_subd_verts
      tot_dist_cur_subd_to_gt_verts = torch.mean(tot_dist_cur_subd_to_gt_verts) ### mean loss

      lap_ori_mesh = self.calculate_mesh_laplacian(vertices=ori_upsample_vertices, edges=ori_upsample_edges, vertices_dof=ori_dof_vertices)
      lap_last_subd_mesh = self.calculate_mesh_laplacian(vertices=last_subd_verts, edges=ori_upsample_edges, vertices_dof=ori_dof_vertices)
      tot_lap_dist_last_subd_ori = torch.sum((lap_ori_mesh - lap_last_subd_mesh) ** 2, dim=-1).mean(dim=-1).mean()
      if opt.loss.with_laplacian:
        weak_sup_loss = tot_lap_dist_last_subd_ori + tot_dist_cur_subd_to_gt_verts + tot_pts_cd
      else:
        weak_sup_loss = tot_dist_cur_subd_to_gt_verts + tot_pts_cd
      return weak_sup_loss, tot_dist_cur_subd_to_gt_verts, tot_lap_dist_last_subd_ori
