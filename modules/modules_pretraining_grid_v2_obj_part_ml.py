...
class TransformerDecoderGrid(nn.Module):
    ...
    def forward(self, inputs, sequential_context_embeddings=None, mask=None, sequential_context_mask=None, context_window=None):
        ...
        if mask is None:
          ...
          # print("mask", mask.size(),  torch.zeros_like(mask).unsqueeze(-1).size(), inputs.size())
          atten_mask = torch.zeros_like(mask).unsqueeze(-1) + (1. - mask).unsqueeze(1) # bsz x seq_len x seq_len
          atten_mask = atten_mask.unsqueeze(1).contiguous().repeat(1, self.num_heads, 1, 1).contiguous().view(bsz * self.num_heads, seq_length, seq_length).contiguous()
          self.atten_mask = atten_mask

        # mask: bsz x seq_length
        ...
        if context_window > 0: # context window # a positive context dinwod
            ...
            context_atten_mask = torch.from_numpy(context_atten_mask).float().cuda()
            context_atten_mask = context_atten_mask.unsqueeze(0) if len(atten_mask.size()) > 2 else context_atten_mask
            atten_mask = context_atten_mask + atten_mask
...
class VertexModel(nn.Module):
    def __init__(self,
        ...
        self.grid_project_to_logits = nn.ModuleList(
          [
            nn.Linear(self.embedding_dim, self.num_grids_quantization + 5, bias=True),
            nn.Linear(self.embedding_dim, self.num_grids_quantization + 5, bias=True),
            nn.Linear(self.embedding_dim, self.num_grids_quantization + 5, bias=True),
          ]
        )
    ...
    #   grid_coord_embeddings = []
      ...
      pure_grid_content_embedding = grid_content_embedding.clone() ### pure_grid_content_embedding: bsz x seq_length x embedding_size
      ...
      grid_order_coord_xyz_embeddings = grid_order_embedding.contiguous().repeat(bsz, 1, 1).contiguous().unsqueeze(2) + grid_xyz_embeddings + grid_coord_embeddings + pure_grid_content_embedding.unsqueeze(2)
    ...
    #   bsz, grid_length = grid_order_embedding.size(0), grid_order_coord_xyz_embeddings.size(1) - 1
      ...
      bsz = grid_mask.size(0)
      cur_mask = grid_mask.unsqueeze(-1).repeat(1, 1, 3).contiguous().view(grid_mask.size(0), -1).contiguous()
      cur_mask = torch.cat(
        [torch.ones((bsz, self.prefix_key_len), dtype=torch.float32).cuda(), cur_mask], dim=-1
      )
      grid_coord_outputs = self.decoder_grid_coord(grid_order_coord_xyz_embeddings, sequential_context_embeddings=None, mask=cur_mask, context_window=opt.model.context_window * 3) # only use coord for coord decoding
    ...
    def _create_dist_grid_content(self, grid_embedding, grid_content_embedding, temperature=1., top_k=0, top_p=1., grid_mask=None):
      ...
      bsz = grid_mask.size(0)
      cur_mask = torch.cat(
        [torch.ones((bsz, self.prefix_key_len), dtype=torch.float32).cuda(), grid_mask], dim=-1
      )
      grid_content_outputs = self.decoder_grid_content(grid_content_embedding, sequential_context_embeddings=grid_embedding, mask=cur_mask, context_window=opt.model.context_window)
    ...
    def forward(self, batch):
        ...
        grid_xyzs = batch['masked_grid_xyzs'] # n_grids x (3) --> grid_xyzs
        grid_content = batch['masked_grid_content_vocab'] # use content_vocab for prediction and predict content_vocab
        # grid_mask = batch['grid_xyzs_mask'] # enough
        grid_mask = batch['grid_content_vocab_mask'] # enough
        ...
        # pred_dist_grid_xyz = self._create_dist_grid_coord_v2(grid_order_coord_xyz_embeddings,  temperature=1., top_k=0, top_p=1.0, grid_mask=grid_mask)
        pred_dist_grid_xyz = self._create_dist_grid_coord_v2(grid_order_coord_xyz_embeddings,  temperature=1., top_k=0, top_p=1.0, grid_mask=grid_mask)
        # create dist...
        # pred_dist_grid_values = self._create_dist_grid_content(grid_embedding, grid_content_embedding, grid_mask=grid_mask)
        pred_dist_grid_values = self._create_dist_grid_content(None, grid_content_embedding, grid_mask=grid_mask)
    ...
    def sample(self, num_samples, context=None, max_sample_length=None, temperature=1., top_k=0, top_p=0.95, recenter_verts=True, only_return_complete=True, cond_context_info=False, sampling_max_num_grids=-1): # only the largets value is considered?
        ...
        grid_xyzs = context['grid_xyzs'] # n_grids x (3) --> grid_xyzs
        grid_content = context['grid_content_vocab'] # use content_vocab for prediction and predict content_vocab
        # grid_mask = context['grid_xyzs_mask'] # enough

        mask_indicators = context['mask_grid_indicators']
        pred_dist_grid_xyz, pred_dist_grid_values = self(context)
        pred_grid_xyz = pred_dist_grid_xyz.sample()
        pred_grid_values = pred_dist_grid_values.sample()

        grid_xyzs = mask_indicators.unsqueeze(-1) * pred_grid_xyz.float() + (1. - mask_indicators.unsqueeze(-1)) * grid_xyzs.float()
        grid_xyzs = grid_xyzs.long()

        grid_content = mask_indicators * pred_grid_values.float() + (1. - mask_indicators) * grid_content.float()
        grid_content = grid_content.long()
        ...
        # grid_xyzs = context['masked_grid_xyzs'] # n_grids x (3) --> grid_xyzs
        # grid_content = context['masked_grid_content_vocab'] # use content_vocab for prediction and predict content_vocab
        # grid_mask = context['grid_xyzs_mask'] # enough
