...
class TransformerDecoderGrid(nn.Module):
    ...
    def forward(self, inputs, sequential_context_embeddings=None, mask=None, sequential_context_mask=None, context_window=None):
        ...
        if mask is None:
          ...
          # atten_mask = mask
          # mask: bsz x seq_length (total sequence length)
          # mask = 1 means no attention
          # [bsz x seq_length x 1] + [bsz x 1 x seq_length] --> [bsz x seq_length x seq_length] # 
          # content mask; l
          atten_mask = torch.zeros_like(mask).unsqueeze(-1) + mask.unsqueeze(1)
        ...
        for i in range(self.num_layers): # num_layers
            ...
            if sequential_context_embeddings is not None:
                ...
                if self.layer_norm:
                    res = self.context_norm_layers[i](res)
                # contents attent to [xyz indicator, order indicator, xyz coordinate indicator]
...
class VertexModel(nn.Module):
    def __init__(self,
        ...
        self.embedding_dim = self.decoder_config['hidden_size'] # vertex model
        ...
        #### grid_embedding_layers for grid_position indicators embedding; grid coordiante embedding', and grid xyz indicator embedding ####
        grid_embedding_layers_list = [nn.Embedding(num_embeddings=self.grid_pos_embed_max_num + 5, embedding_dim=self.embedding_dim)]
        if self.grid_size > 1:
            grid_coord_embedding_layers_list = [nn.Embedding(num_embeddings=self.num_grids_quantization + 5, embedding_dim=self.embedding_dim) for _ in range(3)]
            grid_embedding_layers_list += grid_coord_embedding_layers_list
        grid_embedding_layers_list += [nn.Embedding(num_embeddings=3, embedding_dim=self.embedding_dim)]
        # self.grid_embedding_layers = nn.ModuleList(*grid_embedding_layers_list)
        self.grid_embedding_layers = nn.ModuleList(grid_embedding_layers_list)
    ...
    def _embed_input_grids(self, grid_xyzs, grid_content, grid_pos=None, global_context_embedding=None):
      ...
      if self.grid_size > 1:
        ...
        #   print(f"grid_content_embedding: {grid_content_embedding.size()}, grid_embedding: {grid_embedding.size()}")
    ...
    def _create_dist_grid_coord_v2(self, grid_order_coord_xyz_embeddings,  temperature=1., top_k=0, top_p=1.0, grid_mask=None):
    #   bsz, grid_length = grid_order_embedding.size(0), grid_order_coord_xyz_embeddings.size(1) - 1
      ...
      ###### grid coord outputs ######
      grid_coord_outputs = self.decoder_grid_coord(grid_order_coord_xyz_embeddings, sequential_context_embeddings=None, context_window=opt.model.context_window * 3, mask=grid_mask) # only use coord for coord decoding
    ...
    def _create_dist_grid_content(self, grid_embedding, grid_content_embedding, temperature=1., top_k=0, top_p=1., grid_mask=None):
      ...
      grid_content_outputs = self.decoder_grid_content(grid_content_embedding, sequential_context_embeddings=grid_embedding, context_window=opt.model.context_window, mask=grid_mask)
    ...
    def forward(self, batch):
      #### for sampling, we can also directly forward the batch?
        ...
        grid_xyzs = batch['mask_grid_xyzs']
        # grid_content = batch['grid_content_vocab'] # use content_vocab for prediction and predict content_vocab
        grid_content = batch['mask_grid_content']

        # grid_mask = batch['grid_mask']
        # grid_mask: bsz x grid_length
        grid_mask = batch['grid_content_vocab_mask'] # grid_mask 
        grid_mask = torch.cat(
            [torch.ones((grid_mask.size(0), self.prefix_key_len), dtype=torch.float32).cuda(), grid_mask], dim=-1
        )
        grid_mask = 1.0 - grid_mask

        grid_coord_mask = batch['grid_content_vocab_mask'] # grid_coord_mask: bsz x grid_length
        grid_coord_mask = grid_coord_mask.unsqueeze(-1).contiguous().repeat(1, 1, 3).view(grid_coord_mask.size(0), -1).contiguous() # bsz x (grid_length x 3)
        grid_coord_mask = torch.cat(
            [torch.ones((grid_coord_mask.size(0), self.prefix_key_len), dtype=torch.float32).cuda(), grid_coord_mask], dim=-1
        )
        grid_coord_mask = 1.0 - grid_coord_mask
        ...
        pred_dist_grid_xyz = self._create_dist_grid_coord_v2(grid_order_coord_xyz_embeddings,  temperature=1., top_k=0, top_p=1.0, grid_mask=grid_coord_mask)
        # create dist...

        if self.grid_size > 1:
            pred_dist_grid_values = self._create_dist_grid_content(grid_embedding, grid_content_embedding, grid_mask=grid_mask)
        else:
    ...
    def sample(self, num_samples, context=None, max_sample_length=None, temperature=1., top_k=0, top_p=0.95, recenter_verts=True, only_return_complete=True): # only the largets value is considered?
        ...
        # grid_xyzs = context['grid_xyzs'] # n_grids x (3) --> grid_xyzs; grid_xyzs
        
        # grid_content = context['grid_content_vocab'] # use content_vocab for prediction and predict content_vocab; grid_content
        grid_content = context['mask_grid_content']
        grid_xyzs = context['mask_grid_xyzs']

        # grid_mask = context['grid_mask']
        grid_mask = context['grid_content_vocab_mask']
        grid_mask_indicator = context['mask_grid_indicators'] # bsz x seq_length (1 represents masked tokens)
        ...
        grid_coord_mask = context['grid_content_vocab_mask'] # grid_coord_mask: bsz x grid_length
        grid_coord_mask = grid_coord_mask.unsqueeze(-1).contiguous().repeat(1, 1, 3).view(grid_coord_mask.size(0), -1).contiguous() # bsz x (grid_length x 3)
        grid_coord_mask = torch.cat(
            [torch.ones((grid_coord_mask.size(0), self.prefix_key_len), dtype=torch.float32).cuda(), grid_coord_mask], dim=-1
        )
        ...
        if 'grid_pos' in context:
            grid_pos = context['grid_pos']
        else:
            grid_pos = None
        ...
        sampled_grid_xyz = pred_dist_grid_xyz.sample()
        #### sampled_grid_xyzs: bsz x seq_length x 3
        sampled_grid_xyz = grid_mask_indicator.unsqueeze(-1) * sampled_grid_xyz.float() + (1. - grid_mask_indicator.unsqueeze(-1)) * grid_xyzs.float() ##### grid

        if self.grid_size > 1:
            pred_dist_grid_values = self._create_dist_grid_content(grid_embedding, grid_content_embedding, grid_mask=grid_mask)
            sampled_grid_content = pred_dist_grid_values.sample() # bsz x seq_length (grid contents)
            sampled_grid_content = grid_mask_indicator * sampled_grid_content.float() + (1. - grid_mask_indicator) * grid_content.float() # bsz x seq_length (sampled grid contents)
        else:
            sampled_grid_content = torch.ones_like(grid_content)
        
        # grid_xyzs = grid_xyzs - 1
        grid_xyzs = sampled_grid_xyz - 1
        grid_values = sampled_grid_content
