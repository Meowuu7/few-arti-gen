...
class URDFDataset(data.Dataset):
    def __init__(self, root_folder, quantization_bits=8, nn_max_vertices=8900, nn_max_faces=8900,
                 ...
                 part_names=None, category_nm_to_part_nm=None, is_training=False, padding_to_same=True
                 ):
        ...
        self.padding_to_same = padding_to_same
        ...
        assert self.num_parts > 1, "Use more than one parts at least."
        print(f"In the multi-parts setting with part number: {self.num_parts}.")
        ...
        self.category_part_indicator_to_idxes = category_part_indicator_to_idxes # balanced category_name_to_real_mesh_dict_index
        ...
        print(f"category_name_to_mesh_dict_idxes: {self.category_name_to_mesh_dict_idxes.keys()}")
        print(f"category_part_indicator_to_mesh_dict_idxes: {self.category_part_indicator_to_mesh_dict_idxes.keys()}")
    ...
    def convert_to_grid_xyz(self, vertices):
      # vertices: n_verts
      ...
      for i_v in range(vertices.size(0)): # vertices
        cur_vert_xyz = vertices[i_v].long().tolist()
    ...
    def __getitem__(self, item):
        ...
        if not isinstance(self.mesh_dicts[item], dict): # mesh dict
            self.mesh_dicts[item] = dataset_utils.read_binvox_to_pts(self.mesh_dicts[item])

        # category_part_nm indicator... # mesh dict idx # part name indicator
        category_part_nm_indicator = self.mesh_dict_idx_to_category_name[item]
        # interpretation; inter. indicator' interpretation; intermediate indicators; indicators; use flat coordinates and indicators; we should also use a different pos indicator? and jsut use the context window; positional encoding
        # part section indicators
        # relative/absolute positional encoding
        ...
        if self.padding_to_same: ###### padding_to_same = True: at training time
          cur_category_part_idxes = self.category_part_indicator_to_mesh_dict_idxes[category_part_nm_indicator] # total category part idxes list
          nn_other_parts = self.num_parts - 1 # nn_other_parts
          other_parts_idxes = np.random.choice(len(cur_category_part_idxes), size=nn_other_parts, replace=True).tolist()
          other_parts_idxes = [cur_category_part_idxes[ii] for ii in other_parts_idxes]
          if self.balance_classes:
            other_parts_idxes = [self.balanced_mesh_dicts[ii] for ii in other_parts_idxes]
          for ii in other_parts_idxes: # other 
            if not isinstance(self.mesh_dicts[ii], dict):
              self.mesh_dicts[ii] = dataset_utils.read_binvox_to_pts(self.mesh_dicts[ii])

          other_parts_category_names = [self.mesh_dict_idx_to_category_name[ii] for ii in other_parts_idxes]
          for i_other, other_cat_nm in enumerate(other_parts_category_names):
            assert other_cat_nm == category_part_nm_indicator, f"i_other: {i_other}, current category part name indicator: {category_part_nm_indicator}, other part's category part name indicator: {other_cat_nm}. Same indicators expected!"
          # print(f"current part category name: {category_part_nm_indicator}, other parts category names: {other_parts_category_names}")
          other_parts_dicts = [self.mesh_dicts[ii] for ii in other_parts_idxes] # mesh_dicts[ii] # mesh dicts; model size issue, batch size issue
        

          parts_dicts = [cur_item_mesh_dict] + other_parts_dicts
        else: ###### padding_to_same = False: at test time
          parts_dicts = [cur_item_mesh_dict] 

        tot_grid_xyzs = []
        tot_grid_pts = []
        tot_grid_pos = [] # grid pos
        tot_grid_content = []
        # tot_flat_grid_pos = 

        tot_num_grids = 0
        # 
        for i_part, cur_part_mesh_dict in enumerate(parts_dicts):
          # make vertices; long vertices
          vertices = torch.from_numpy(cur_part_mesh_dict['vertices']).long()
          #### scale current vertices ####
          ...
          # grid_size = opt.vertex_model.grid_size
          # grid_pts: n_grids x grid_size x grid_size x grid_size; grid_xyzs: n_grids x 3; grid_xyzs: n_grids x 3
          # #### pts and xyzs ####
          grid_pts, grid_xyzs = self.convert_to_grid_xyz(vertices_permuted) # grid_pts: n_grids x ...
          # # grid_xyzs: n_grids x 3
          # flat_grid_xyzs = grid_xyzs.contiguous().view(-1).contiguous() # (n_grids x 3)
          # tot_flat_grid_xyzs.append(flat_grid_xyzs)
          # if i_part < self.num_parts - 1:
          #   tot_flat_grid_xyzs.append(torch.tensor([-2], dtype=torch.long).long())
          # else:
          #   tot_flat_grid_xyzs.append(torch.tensor([-1], dtype=torch.long).long())
          
          # grid content
          # content of grids
          grid_content = self.convert_grid_values_to_grid_content(grid_pts) # nn_grids

          grid_pos = torch.arange(start=0, end=grid_pts.size(0), step=1, dtype=torch.long) # grid pos
          # flat_grid_pos = grid_pos.contiguous().unsqueeze(-1).repeat(1, 3).contiguous().view(-1).contiguous()
          # tot_flat_grid_pos.append(flat_grid_pos)
          grid_pos = grid_pos + tot_num_grids # 
          tot_num_grids += grid_pos.size(0)

          nn_grids = grid_pts.size(0) # nn_grids in the grids
          # print(f"nn_grids: {nn_grids}, max_num_grids: {self.max_num_grids}, grid_size: {self.grid_size}")

          tot_grid_xyzs.append(grid_xyzs)
          tot_grid_pts.append(grid_pts)
          tot_grid_pos.append(grid_pos)
          tot_grid_content.append(grid_content)


        tot_grid_xyzs = torch.cat(tot_grid_xyzs, dim=0)
        tot_grid_pts = torch.cat(tot_grid_pts, dim=0)
        tot_grid_pos = torch.cat(tot_grid_pos, dim=0) # 
        tot_grid_content = torch.cat(tot_grid_content, dim=0)
        nn_grids = tot_grid_pos.size(0)
        grid_pos = tot_grid_pos
        grid_xyzs = tot_grid_xyzs
        grid_pts = tot_grid_pts
        grid_content = tot_grid_content

        ###### if there are more than permit grids in the sample ######
        if nn_grids >= self.max_num_grids: # max_num_grids
          ...
          else: # not start from zero
            grid_pos = grid_pos[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          ...
          if cut_grid_st_pos == nn_grids - self.max_num_grids + 1:
            ...
            grid_xyzs_mask = torch.cat(
              [torch.ones((grid_xyzs.size(0), 3), dtype=torch.float32), torch.tensor([[0, 0, 0]], dtype=torch.float32), torch.zeros((self.max_num_grids - grid_xyzs.size(0) - 1, 3), dtype=torch.float32)], dim=0
            )
        else:
          grid_xyzs_mask = torch.cat(
            [torch.ones((grid_xyzs.size(0), 3), dtype=torch.float32), torch.tensor([[1, 0, 0]], dtype=torch.float32), torch.zeros((self.max_num_grids - grid_xyzs.size(0) - 1, 3), dtype=torch.float32)], dim=0
        ...
        if self.padding_to_same:
          # grid_pts_mask = torch.cat(
          #   [torch.ones((grid_pts.size(0), self.grid_size, self.grid_size, self.grid_size), dtype=torch.float32), torch.zeros((self.max_num_grids - grid_xyzs.size(0), self.grid_size, self.grid_size, self.grid_size), dtype=torch.float32)], dim=0
          # )

          # grid_content_mask = torch.cat(
          #     [torch.ones((grid_content.size(0), ), dtype=torch.float32), torch.zeros((self.max_num_grids - grid_content.size(0), ), dtype=torch.float32)], dim=0
          # )
          ...
          # grid_xyzs: n_grids x 3 ##### grid_xyzs #####

          # grid_xyzs = torch.cat(
          #   [grid_xyzs + 1, torch.zeros((self.max_num_grids - grid_xyzs.size(0), 3), dtype=torch.long)], dim=0
          # )
