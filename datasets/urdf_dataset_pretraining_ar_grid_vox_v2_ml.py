...
class URDFDataset(data.Dataset):
    def __init__(self, root_folder, quantization_bits=8, nn_max_vertices=8900, nn_max_faces=8900,
                 ...
                 part_names=None, category_nm_to_part_nm=None, is_training=False
                 ):
    ...
    def create_random_grid_content_mask(self, grid_content, grid_xyzs):
      # grid_content: bsz x grid_length
      nn_grids = grid_content.size(-1)
      mask_ratio_low = opt.dataset.mask_low_ratio
      mask_ratio_high = opt.dataset.mask_high_ratio
      mask_ratio = np.random.uniform(low=mask_ratio_low, high=mask_ratio_high, size=(1,)).item()
      mask_nn_unflat = int(mask_ratio * nn_grids)
      mask_nn_unflat = max(mask_nn_unflat, 1)


      nn_div_mask_window = 10

      mask_window = 100
      tot_mask_window = max(1, mask_nn_unflat // mask_window)
      sampling_p = np.ones((nn_grids, ), dtype=np.float32)
      sampling_p[nn_grids - mask_window // nn_div_mask_window: ] = 0.
      sampling_p = sampling_p / np.sum(sampling_p)
      sampled_mask_window_st_poses = []
      for i_w in range(tot_mask_window):
        cur_sampled_st_pos = np.random.choice(nn_grids, size=(1,),p=sampling_p).item()
        sampled_mask_window_st_poses.append(cur_sampled_st_pos)
        sampling_p[cur_sampled_st_pos: min(cur_sampled_st_pos + tot_mask_window // nn_div_mask_window, nn_grids)] = 0.
        if np.sum(sampling_p).item() < 1e-5:
          break
        sampling_p = sampling_p / np.sum(sampling_p).item()

      masked_grid_content = grid_content.clone()
      masked_grid_xyzs = grid_xyzs.clone()
      mask_grid_indicators = torch.zeros((nn_grids, ), dtype=torch.float32)
      
      # if len()
      for i_w, cur_sampled_st_pos in enumerate(sampled_mask_window_st_poses):
        cur_st_pos, cur_ed_pos = cur_sampled_st_pos, min(cur_sampled_st_pos + mask_window, nn_grids)
        mask_grid_indicators[cur_st_pos: cur_ed_pos] = 1.
        masked_grid_xyzs[cur_st_pos: cur_ed_pos] = MASK_COORD_VALUE
        masked_grid_content[cur_st_pos: cur_ed_pos] = MASK_GRID_VALIE
        mask_grid_indicators[cur_st_pos: cur_ed_pos] = 1.
      

      ######## mask grids #########
    #   sampled_mask_grids_indices = np.random.choice(nn_grids, size=mask_nn_unflat, replace=False)

    #   sampled_mask_grids_indices = torch.from_numpy(sampled_mask_grids_indices).long()  # maksed coord

    #   mask_grid_indicators = torch.zeros_like(grid_content)

        mask_grid_content = masked_grid_content
        mask_grid_xyzs = masked_grid_xyzs
    #   mask_grid_content = grid_content.clone()
    #   mask_grid_content[sampled_mask_grids_indices] = MASK_GRID_VALIE
    #   mask_grid_indicators[sampled_mask_grids_indices] = 1. # mask indicators 
      return mask_grid_content, mask_grid_xyzs, mask_grid_indicators
    ...
    def __getitem__(self, item):
        ...
        ###### mask_grid_xyzs: grid_length x 3
        ###### mask_grid_content: grid_length

        # mask_grid_content, mask_grid_xyzs, mask_grid_indicators = self.create_random_grid_content_mask(grid_content, grid_xyzs)

        # grid_pos = torch.arange(start=0, end=grid_pts.size(0), step=1, dtype=torch.long) # grid pos

        nn_grids = grid_pts.size(0)
        ...
        if nn_grids >= self.max_num_grids:
          ...
          ##### truncate grid xyzs and mask grid xyzs #####
          ...
          # grid_pos = torch.arange(start=0, end=grid_pts.size(0), step=1, dtype=torch.long) 
          ...
          # mask_grid_xyzs = mask_grid_xyzs[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          #### cut grid content and grid indicators ####
          # mask_grid_content = mask_grid_content[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          # mask_grid_indicators = mask_grid_indicators[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          #### cut grid content and grid indicators ####

          #### prediction strategy and others ####
          ...
          mask_grid_content, mask_grid_xyzs, mask_grid_indicators = self.create_random_grid_content_mask(grid_content, grid_xyzs) #### grid_xyzs
        ...
        mask_grid_xyzs = torch.cat(
          [mask_grid_xyzs + 1, torch.zeros((self.max_num_grids - mask_grid_xyzs.size(0), 3), dtype=torch.long)], dim=0
        )
        ...
        grid_content = torch.cat(
          [grid_content, torch.zeros((self.max_num_grids - grid_content.size(0), ), dtype=torch.long) + MASK_GRID_VALIE + 1], dim=0
        )

        ##### mask grid content #####
        mask_grid_content = torch.cat(
          [mask_grid_content, torch.zeros((self.max_num_grids - mask_grid_content.size(0), ), dtype=torch.long) + MASK_GRID_VALIE + 1], dim=0
        )

        mask_grid_indicators = torch.cat(
          [mask_grid_indicators, torch.zeros((self.max_num_grids - mask_grid_indicators.size(0), ), dtype=torch.float32)], dim=0
        )
        ...
        rt_dict = {
            ...
            'mask_grid_xyzs': mask_grid_xyzs,
            # 'grid_pts_flat': grid_pts_flat,
            'grid_content': grid_pts,
            'mask_grid_content': mask_grid_content,
            'mask_grid_indicators': mask_grid_indicators,
