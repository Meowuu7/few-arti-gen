...
from utils.constants import ENDING_GRID_VALUE, ENDING_XYZ, MASK_COORD_VALUE, MASK_GRID_VALIE, PART_SEP_GRID_VALUE, PART_SEP_XYZ
...
class URDFDataset(data.Dataset):
    ...
    def generate_masks(self, grid_xyzs, grid_content):
      ...
      masked_grid_xyzs, masked_grid_content = grid_xyzs.clone(), grid_content.clone()
      ...
      for i_w, cur_sampled_st_pos in enumerate(sampled_mask_window_st_poses):
        ...
        mask_grid_indicators[cur_st_pos: cur_ed_pos] = 1.
      if torch.sum(mask_grid_indicators).item() < 1e-5:
        print(f"total number of masked grids: {torch.sum(mask_grid_indicators).item()}, grid_xyzs: {grid_xyzs.size()}, grid_content: {grid_content.size()}")
      return masked_grid_xyzs, masked_grid_content, mask_grid_indicators
    ...
    def __getitem__(self, item):
        ...
        masked_grid_xyzs, masked_grid_content, mask_grid_indicators = self.generate_masks(grid_xyzs, grid_content)
        ...
        if nn_grids >= self.max_num_grids:
          ...
          masked_grid_xyzs = masked_grid_xyzs[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          masked_grid_content = masked_grid_content[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
        ...
        masked_grid_xyzs = masked_grid_xyzs + 1 # maks grid xyzs adn grid content
        ...
        if self.is_training:
          ...
          masked_grid_xyzs = torch.cat(
            [masked_grid_xyzs, torch.zeros((self.max_num_grids - masked_grid_xyzs.size(0), 3), dtype=torch.long)], dim=0
          )
          ...
          ##### MASK_GRID_VALUE for padding contents #####
          grid_content = torch.cat(
            [grid_content, torch.zeros((self.max_num_grids - grid_content.size(0), ), dtype=torch.long) + MASK_GRID_VALIE], dim=0
          )

          masked_grid_content = torch.cat(
            [masked_grid_content, torch.zeros((self.max_num_grids - masked_grid_content.size(0), ), dtype=torch.long) + MASK_GRID_VALIE], dim=0
          )

          #### mask grid 
          mask_grid_indicators = torch.cat(
            [mask_grid_indicators, torch.zeros((self.max_num_grids - mask_grid_indicators.size(0), ), dtype=torch.long)], dim=0
          )
        ...
        rt_dict = {
            ...
            'masked_grid_xyzs': masked_grid_xyzs,
            'masked_grid_content_vocab': masked_grid_content,
