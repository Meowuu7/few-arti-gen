...
class URDFDataset(data.Dataset):
    def __init__(self, root_folder, quantization_bits=8, nn_max_vertices=8900, nn_max_faces=8900,
        ...
        print(f"Using num_parts: {self.num_parts}...")
    ...
    def convert_to_grid_xyz(self, vertices, last_part=False): #### convert to grid xyzs 
      # vertices: n_verts
      ...
      tot_grid_xyzs = torch.tensor(tot_grid_xyzs, dtype=torch.long) 
      tot_grid_xyzs = [tot_grid_xyzs]

      if last_part:
        ending_grid_xyzs = torch.tensor(ENDING_XYZ, dtype=torch.long)
        ##### ending grid xyz #####
        tot_grid_xyzs.append(ending_grid_xyzs.unsqueeze(0))
        ##### ending grid pts #####
        ...
        tot_grid_pts.append(ending_grid_pts)
      else:
        ending_grid_xyzs = torch.tensor(PART_SEP_XYZ, dtype=torch.long)
        ##### part sep grid xyz #####
        tot_grid_xyzs.append(ending_grid_xyzs.unsqueeze(0))
        # ending_grid_pts = data_utils.convert_grid_content_to_grid_pts(ENDING_GRID_VALUE, grid_size=self.grid_size)
        ##### part sep grid pts #####
        ending_grid_pts = torch.tensor([PART_SEP_GRID_VALUE], dtype=torch.long)
        tot_grid_pts.append(ending_grid_pts)

      # grid_content_mask = torch.ones(())
    ...
    ##### grid values (gs x gs x gs) --> grid_content (1,)
    ...
    #### we should add a part seperation item here

    ''' Load data with random axises flipping and random vertices shifting  '''
    def __getitem__(self, item):

        if self.balance_classes: # balance classes
        ...
        tot_sample_items = [item]
        if self.num_parts > 1 and self.is_training:
          cur_cat_part_mesh_dicts_idxes = self.category_name_to_mesh_dict_idxes[category_part_nm_indicator]
          #### cur_cat_part_mesh_dicts_idxes
          other_sampled_items = np.random.choice(cur_cat_part_mesh_dicts_idxes, size=(self.num_parts - 1, ), replace=True).tolist()
          other_sampled_items = [self.balanced_mesh_dicts[other_item] for other_item in other_sampled_items]
          tot_sample_items += other_sampled_items

        for cur_item in tot_sample_items:
          if not isinstance(self.mesh_dicts[cur_item], dict):
              self.mesh_dicts[cur_item] = dataset_utils.read_binvox_to_pts(self.mesh_dicts[cur_item])
        
        tot_grid_pts = []
        tot_grid_xyzs = []
        tot_grid_content_mask = []
        
        for i_item, cur_item in enumerate(tot_sample_items):

          # category_part_nm indicator...
          category_part_nm_indicator = self.mesh_dict_idx_to_category_name[cur_item]
          ...
          vertices = torch.from_numpy(cur_item_mesh_dict['vertices']).long() # vertices
          ...
          last_part = (i_item == len(tot_sample_items) - 1 ) and self.is_training 
          # grid_pts: n_grids x grid_size x grid_size x grid_size; grid_xyzs: n_grids x 3
          grid_pts, grid_xyzs, grid_content_mask = self.convert_to_grid_xyz(vertices_permuted, last_part=last_part) # grid_pts: n_grids x ...
          ...
          tot_grid_content_mask.append(grid_content_mask)
        
        grid_pts = torch.cat(tot_grid_pts, dim=0) #### grid_pts
        grid_xyzs = torch.cat(tot_grid_xyzs, dim=0) #### grid_xyzs
        grid_content_mask = torch.cat(tot_grid_content_mask, dim=0)
        ...
        if nn_grids >= self.max_num_grids: ##### nn_grids and xxx
        #   if not self.is_training:
        #     print(f"Using max_num_grids: {self.max_num_grids}, current nn_grids: {nn_grids}...")
          ...
          if self.use_context_window_as_max:
            grid_pos = grid_pos[ :self.max_num_grids - 1] ##### cut grid_pos to make it start from zero...
        ...
        if self.is_training:
            grid_xyzs_mask = torch.cat(
                [grid_xyzs_mask, torch.tensor([[0, 0, 0]], dtype=torch.float32), torch.zeros((self.max_num_grids - grid_xyzs_mask.size(0) - 1, 3), dtype=torch.float32)], dim=0
            )
        
            
            grid_content_mask = torch.cat(
                [grid_content_mask, torch.zeros((self.max_num_grids - grid_content_mask.size(0), ), dtype=torch.float32)], dim=0
            )
