...
class URDFDataset(data.Dataset):
    ...
    def convert_to_grid_xyz(self, vertices, pts_idxes=None):
      ...
      for i_part, cur_part_pts_idxes in enumerate(pts_idxes):
        ...
        tot_grid_pts = []
        tot_grid_pos = [] # vertices positions
        tot_grid_xyz_indicators = [] # grids' xyz indicators (5 indicators with indicators for inter-part separation and object ending)
        tot_grid_xyzs = []
        ...
        for cur_item in sorted_grid_points:
          ...
          pts_idx_to_st_idx[tot_nn_real_pts] = tot_nn_xyz_len

          # tot_grid_xyzs.append(cur_item[0]) # sorted 
          tot_grid_xyzs += [cur_item[0][0], cur_item[0][1], cur_item[0][2]]
          tot_grid_pos += [tot_nn_pts, tot_nn_pts, tot_nn_pts]
          tot_grid_xyz_indicators += [0, 1, 2]
          
          tot_nn_pts += 1
          tot_nn_xyz_len += 3
          tot_nn_real_pts += 1
        ##### add information for inter-part indicators ---> positional values, positional indicators, and xyz indicators
        if i_part < len(pts_idxes) - 1:
          tot_grid_xyzs += [-1]
          tot_grid_pos += [tot_nn_pts]
          tot_nn_pts += 1
          tot_nn_xyz_len += 1
          tot_grid_xyz_indicators += [3] # grid indicators
        ...
        tot_grid_pos = torch.tensor(tot_grid_pos, dtype=torch.long)
        tot_grid_xyz_indicators = torch.tensor(tot_grid_xyz_indicators, dtype=torch.long)
        
        

        all_grid_xyzs.append(tot_grid_xyzs)
        all_grid_pts.append(tot_grid_pts)
        all_grid_pos.append(tot_grid_pos)
        all_grid_indicators.append(tot_grid_xyz_indicators)
      
      all_grid_xyzs.append(torch.tensor([ -2], dtype=torch.long))
      all_grid_pos.append(torch.tensor([tot_nn_pts], dtype=torch.long))
      all_grid_indicators.append(torch.tensor([4], dtype=torch.long))

      #### tot_nn_real_pts: final number of real points; 
      pts_idx_to_st_idx[tot_nn_real_pts] = tot_nn_xyz_len + 1
      
      tot_grid_pts = torch.cat(all_grid_pts, dim=0)
      tot_grid_xyzs = torch.cat(all_grid_xyzs, dim=0)
      tot_grid_pos = torch.cat(all_grid_pos, dim=0)
      tot_grid_indicators = torch.cat(all_grid_indicators, dim=0)
      return tot_grid_pts, tot_grid_xyzs, tot_grid_pos, tot_grid_indicators, pts_idx_to_st_idx
    ...
    def __getitem__(self, item):
        ...
        #### if we use 001 to represent part spe and 000 for end of the object ####
        ...
        grid_pts, grid_xyzs, grid_pos, grid_indicators, pts_idx_to_st_idx = self.convert_to_grid_xyz(vertices_permuted, pts_idxes) # grid_pts: n_grids x ... ##### pts_idxes!
        ...
        if nn_grids >= self.max_num_grids:
          ...
          xyz_st_idx = pts_idx_to_st_idx[cut_grid_st_pos]
          xyz_ed_idx = pts_idx_to_st_idx[cut_grid_st_pos + self.max_num_grids - 1]
          ...
          grid_xyzs = grid_xyzs[xyz_st_idx: xyz_ed_idx]
          # grid_pos = grid_pos[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          grid_pos = grid_pos[xyz_st_idx: xyz_ed_idx]
          grid_indicators  = grid_indicators[xyz_st_idx: xyz_ed_idx]
          ...
          if cut_grid_st_pos == nn_grids - self.max_num_grids + 1:
            grid_xyzs_mask = torch.cat(
              [torch.ones((grid_xyzs.size(0), ), dtype=torch.float32), torch.zeros((self.max_num_grids * 3 - grid_xyzs.size(0), ), dtype=torch.float32)], dim=0
            )
          else:
            grid_xyzs_mask = torch.cat(
              [torch.ones((grid_xyzs.size(0), ), dtype=torch.float32), torch.zeros((self.max_num_grids * 3 - grid_xyzs.size(0), ), dtype=torch.float32)], dim=0
          ...
          grid_xyzs_mask = torch.cat(
            [torch.ones((grid_xyzs.size(0), ), dtype=torch.float32), torch.zeros((self.max_num_grids * 3 - grid_xyzs.size(0), 3), dtype=torch.float32)], dim=0
          )
        ...
        ##### grid_xyzs ---> 
        grid_xyzs = torch.cat(
          [grid_xyzs + 2, torch.zeros((self.max_num_grids * 3 - grid_xyzs.size(0), ), dtype=torch.long)], dim=0
        )
        ...
        ##### grid_pos: j
        grid_pos = torch.cat(
          [grid_pos, torch.zeros((self.max_num_grids * 3 - grid_pos.size(0)), dtype=torch.long)
          ], dim=0
        )
        ...
        rt_dict = {
            ...
            'grid_xyzs': grid_xyzs, ##### [discrete xyz values] + [Part SEP indicators] + [Ending indicator]
            'grid_xyzs_mask': grid_xyzs_mask, ##### [discrete xyz values] + [Part SEP indicators] + [Ending indicator]
