...
class URDFDataset(data.Dataset):
    def __init__(self, root_folder, quantization_bits=8, nn_max_vertices=8900, nn_max_faces=8900,
        ...
        # self.statistic_folder = statistic_folder ##### part_names
    ...
    def convert_to_grid_xyz(self, vertices):
      ...
      half_part = opt.dataset.half_part
      self.grid_xyz_to_points = {}
      for i_v in range(vertices.size(0)): # vertices # the first coordiante
      ...
      if half_part:
        tot_grid_mask = torch.zeros_like(tot_grid_xyzs[:,0]).float()
        maxx_tot_grid_x, minn_tot_grid_x = torch.max(tot_grid_xyzs[:, 0]).item(), torch.min(tot_grid_xyzs[:, 0]).item()
        mid_tot_grid_x = (maxx_tot_grid_x + minn_tot_grid_x) // 2
        tot_grid_mask[tot_grid_xyzs[:, 0] <= mid_tot_grid_x] = 1.
      else:
        tot_grid_mask = torch.ones_like(tot_grid_xyzs[:, 0]).float()
      return tot_grid_pts, tot_grid_xyzs, tot_grid_mask
    ...
    def __getitem__(self, item):
        ...
        grid_pts, grid_xyzs, grid_mask = self.convert_to_grid_xyz(vertices_permuted) # grid_pts: n_grids x ...
        ...
        grid_xyzs_mask = grid_mask.unsqueeze(-1).contiguous().repeat(1, 3).contiguous()
        grid_pts_mask = grid_mask.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, self.grid_size, self.grid_size, self.grid_size).contiguous()
        ...
        if nn_grids >= self.max_num_grids:
          ...
          grid_xyzs_mask = grid_xyzs_mask[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          grid_pts_mask = grid_pts_mask[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
          grid_mask = grid_mask[cut_grid_st_pos: cut_grid_st_pos + self.max_num_grids - 1]
        ...
        # )
        grid_content_mask = torch.cat(
            [grid_mask, torch.zeros((self.max_num_grids - grid_content.size(0), ), dtype=torch.float32)], dim=0
        )
