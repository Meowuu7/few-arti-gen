...
class URDFDataset(data.Dataset):
    def __init__(self, root_folder, quantization_bits=8, nn_max_vertices=8900, nn_max_faces=8900,
        ...
        if datasest_name in ["MotionDataset", "PolyGen_Samples"]:
            ...
            for cur_cat_nm in category_name:
                ...
                cur_root_folder, split='train')
                print(f"Pretraining dataset, start loading data for the category {cur_cat_nm} with parts {part_names}. Split: {'train'}. ")
        ...
        print(f"category_part_indicator_to_idxes: {self.category_part_indicator_to_idxes}")
        ...
        print(f"Returning with dataset_len: {self.dataset_len}")
    ...
    def __getitem__(self, item):
        ...
        if not isinstance(self.mesh_dicts[item], dict):
            ...
            self.mesh_dicts[item] = data_utils.read_mesh_from_obj_file(self.mesh_dicts[item], quantization_bits=self.quantization_bits, recenter_mesh=True, remove_du=False)
        cur_item_mesh_dict = self.mesh_dicts[item]
