...
class URDFDataset(data.Dataset):
    def __init__(self, root_folder, quantization_bits=8, nn_max_vertices=8900, nn_max_faces=8900,
        ...
        for cur_cat_nm in category_name:
            ...
            print(f"Pretraining dataset, start loading data for the category {cur_cat_nm} with parts {part_names}. Data Tyep: {self.data_type}")
            if self.data_type == 'binvox':
                ...
                cur_cat_mesh_dicts, part_tree, part_nm_to_mesh_dict_idxes = dataset_utils.get_mesh_dict_list_multi_part_mesh_vertices(
                    cur_root_folder, part_names=part_names,
                    ret_part_name_to_mesh_idx=True, remove_du=self.remove_du, split=self.split)
            print(f"Category {cur_cat_nm} with parts {part_names} loaded. Total number: {len(cur_cat_mesh_dicts)}.")

            cur_mesh_dict_len = len(mesh_dicts)
            cur_cat_nn = 0
            for cur_cat_cur_part_nm in part_nm_to_mesh_dict_idxes: # part_nm to mesh_dict_idxes
                new_mesh_dict_idxes = []
    ...
    def __getitem__(self, item):
        ...
        mask_vertices_flat = torch.cat( # mask vertices flat
