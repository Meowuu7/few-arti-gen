...
class URDFDataset(data.Dataset):
    def __init__(self, root_folder, quantization_bits=8, nn_max_vertices=8900, nn_max_faces=8900, nn_max_permite_vertices=400, nn_max_permite_faces=2000, category_name="eyeglasses", instance_nns=[], part_names=None, category_nm_to_part_nm=None
        ...
        if datasest_name in ["MotionDataset", "PolyGen_Samples"]:
          ...
          for cur_cat_nm in category_name:
            ...
            print(f"Finetuning dataset, start loading paired data for the category {cur_cat_nm} with parts {part_names}.")
            cur_cat_mesh_dicts, part_tree = dataset_utils.get_mesh_dict_list_multi_part_part_first_paired_finetuning(cur_root_folder, part_names=part_names, valid_indices=self.instance_nns, category_name=cur_cat_nm)
    ...
    def _generate_vertices_mask(self, vertices_flat):
      ...
      vertices_flat_masked = torch.zeros_like(vertices_flat).long()
    ...
    def _get_item_seq_ordering(self, item):
        ...
        nn_backbone_verts = cur_item_mesh_dict['nn_backbone_verts']

        # Re-order vertex coordinates as (z, y, x). # vertices peermuted
        ...
        vertices_flat_mask = torch.zeros_like(vertices_flat).float()
        vertices_flat_mask[nn_backbone_verts * 3: ] = 1.
        # vertices_

        vertices_mask_identifier = torch.zeros_like(vertices_mask_identifier).float()
        vertices_mask_identifier[nn_backbone_verts * 3: ]  = 1.
        ...
        # vertices_mask_identifier = torch.
        ...
        vertices_mask_identifier = torch.cat(
          [vertices_mask_identifier, torch.zeros((1,), dtype=torch.float32), torch.zeros((self.nn_max_vertices * 3 - nn_vertices - 1), dtype=torch.float32)], dim=-1
        )
    ...
    def __getitem__(self, item):
        # vertices ordering strategy: 1) naive strategy; 2) first and last strategy; 3) backbone-to-details strategy
        cur_item_mesh_dict = self.mesh_dicts[item]
        ...
        # flat vertices
        # vertices_flat_ordered: (nn_vertices_flat,)
        vertices_flat = torch.cat(
          [vertices_flat[:3], vertices_flat[nn_vertices_flat - 3: ],  vertices_flat[3: nn_vertices_flat - 3]], dim=0
        )
        vertices_positional_orders = torch.floor_divide(torch.arange(nn_vertices_flat), 3)
        # vertices_positional_orders: (nn_vertices_flat, )
        vertices_positional_orders = torch.cat(
          [vertices_positional_orders[:3], vertices_positional_orders[nn_vertices_flat - 3:], vertices_positional_orders[3: nn_vertices_flat - 3]], dim=0
        )
        vertices_mask_identifier = torch.ones_like(vertices_flat).float()
        vertices_mask_identifier[:6] = 0.
        # #### use vertices flat ordered as verices_flat
        # #### use vertices_mask_identifier as the masked vertices
        vertices_flat_mask = torch.ones_like(vertices_flat).float()
        vertices_flat_mask[:6] = 0.
        vertices_flat_mask = torch.cat([vertices_flat_mask, torch.ones((1,), dtype=torch.float32)], dim=0)
        # vertices_flat_mask...
        ...
        #### pad vertices positional orders ####
        res_vertices_positional_orders = torch.floor_divide(torch.arange(nn_vertices_flat, self.nn_max_vertices * 3), 3)
        vertices_positional_orders = torch.cat(
          [vertices_positional_orders, res_vertices_positional_orders], dim=0
        )
        #### pad vertices flat mask ####
        res_vertices_flat_mask = torch.zeros((self.nn_max_vertices * 3 - vertices_flat_mask.size(0), ), dtype=torch.float32)
        vertices_flat_mask = torch.cat([vertices_flat_mask, res_vertices_flat_mask], dim=0)
        #### pad vertices mask identifier ####
        res_vertices_mask_identifier = torch.zeros((self.nn_max_vertices * 3 - nn_vertices_flat, ), dtype=torch.float32)
        vertices_mask_identifier  = torch.cat([vertices_mask_identifier , res_vertices_mask_identifier], dim=0)
        ...
        rt_dict = {
            ...
            # 'vertices_permutation': permut,
            # 'vertices_masked_permutation': permut,
            'vertices_positional_orders': vertices_positional_orders
        }
