...
def optimize(data):
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        sampled_pts = sampled_pts.cuda()
        context_feature, _ = pc_encoder_cond_model(None, sampled_pts)
        ...
        fine_data['global_context'] = context_feature
        ...
        vertex_prediction_loss = torch.sum(fine_vertex_prediction_loss)
        ...
        face_prediction_loss = torch.sum(fine_face_prediction_loss)
        ...
        pc_encoder_optimizer.zero_grad()
        loss.backward()
        # face_optimizer.step()
        # vertex_optimizer.step()
        pc_encoder_optimizer.step()
        ...
        if step_counter % check_step == 0:
            ...
            print(f"Fine vertex loss: {fine_vertex_prediction_loss.sum().item()}")
            print(f"Fine face loss: {fine_face_prediction_loss.sum().item()}")
...
def step(train_dataset,  batch_size, dataset_iter):
...
def step_sample(test_dataset, test_dataset_iter):
...
def sample(test_data, cur_step=0):
    ...
    with torch.no_grad():
        ...
        sampled_pts = test_data['sampled_pts']
        sampled_pts = sampled_pts.cuda()
        context_feature, _ = pc_encoder_cond_model(None, sampled_pts)

        fine_test_data['global_context'] = context_feature
...
def save_ckpts(cur_step):
    ...
    pc_encoder_model_sv_folder = os.path.join(sv_ckpts_folder, "pc_encoder_cond_model")
    os.makedirs(pc_encoder_model_sv_folder, exist_ok=True)
    ...
    # model_sv_path = f'{cur_step}.pth'
    pc_encoder_model_sv_pth = os.path.join(pc_encoder_model_sv_folder, model_sv_path)
    pc_encoder_model_params = pc_encoder_cond_model.state_dict()
    torch.save(pc_encoder_model_params, pc_encoder_model_sv_pth) # face model path...
    ...
    print(f"pc_encoder model at step {cur_step} saved to {pc_encoder_model_sv_pth}.")



# train iter
def train_iter(train_dataset, test_dataset, training_steps, batch_size):
    # dataset_iter
    # iter dataset
    dataset_iter = iter(train_dataset)
    test_dataset_iter = iter(test_dataset)
    for i in range(training_steps):
        torch.cuda.empty_cache()
        dataset_iter = step(train_dataset, batch_size, dataset_iter)
        torch.cuda.empty_cache()
        if i % check_step == 0: # then sample from the model and save
            ...
            test_dataset_iter = step_sample(test_dataset, test_dataset_iter)
            save_ckpts(cur_step=i)
...
def resume_from_ckpts(vertex_model, face_model, pc_encoder_cond_model, vertex_model_path, face_model_path, pc_encoder_model_path):
    ...
    pc_encoder_model_state_dicts = torch.load(pc_encoder_model_path, map_location="cpu")

    vertex_model.load_state_dict(vertex_model_state_dicts)
    face_model.load_state_dict(face_model_state_dicts)
    pc_encoder_cond_model.load_state_dict(pc_encoder_model_state_dicts)
...
if __name__=='__main__':
    ...
    pc_encoder_cond_model = PointnetPP(in_feat_dim=3, use_light_weight=opt.model.use_light_weight)

    vertex_model_path = opt.model.vertex_model_path
    face_model_path = opt.model.face_model_path
    pc_encoder_model_path = opt.model.pc_encoder_model_path

    resume_from_ckpts(fine_vertex_model, fine_face_model, pc_encoder_cond_model, vertex_model_path, face_model_path, pc_encoder_model_path)
    # resume_from_ckpts ---> 


    # vertex_model = vertex_model.cuda()
    # face_model = face_model.cuda()
    pc_encoder_cond_model = pc_encoder_cond_model.cuda()
    ...
    pc_encoder_optimizer = optim.Adam(pc_encoder_cond_model.parameters(), lr=learning_rate)
    ...
    # train iters...
    train_iter(dataset_train, dataset_test, training_steps, batch_size)
