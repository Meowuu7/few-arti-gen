...
# from datasets.urdf_dataset_pretraining_xl import URDFDataset as URDFDataset_xl
from datasets.urdf_dataset_pretraining_ar_coarse_to_fine_bak import URDFDataset as URDFDataset_ar
...
from modules.modules_pretraining_ar_coarse_to_fine_bak_with_prompt import VertexModel, FaceModel
from modules.modules_pretraining_ar_coarse_to_fine_bak import VertexModel as VertexModel_np
from modules.modules_pretraining_ar_coarse_to_fine_bak import FaceModel as FaceModel_np
...
def optimize(data, vertex_model, face_model, vertex_optimizer, face_optimizer):
    ...
    coarse_face_model.train()
    coarse_vertex_model.train()

    encoder_adapter.train()
    decoder_adapter_vertex.train()
    decoder_adapter_face.train()

    # coarse_encoder_adapter.train()
    # coarse_decoder_adapter_vertex.train()
    # coarse_decoder_adapter_face.train()
    ...
    coarse_vertex_model_pred_dict, global_feature = coarse_vertex_model(coarse_data, return_global_feature=True)
    coarse_face_model_pred_dict, global_feature = coarse_face_model(coarse_data, rt_global_features=True)
    

    coarse_vertices_global_feature = global_feature
    fine_data['global_context'] = coarse_vertices_global_feature
    
    fine_vertex_model_pred_dict = vertex_model(fine_data, adapter_module=decoder_adapter_vertex)

    #
    # coarse_face_model_pred_dict = face_model(coarse_data)
    fine_face_model_pred_dict = face_model(fine_data, encoder_adapter=encoder_adapter, decoder_adapter=decoder_adapter_face) # fine_face_model
    ...
    coarse_face_optimizer.zero_grad() # zero_grad
    coarse_vertex_optimizer.zero_grad()
    ...
    coarse_face_optimizer.step()
    coarse_vertex_optimizer.step()
...
def sample(test_data, vertex_model, face_model, cur_step=0):
    ...
    with torch.no_grad():
        ...
        coarse_vertex_model_pred_dict, global_feature = coarse_vertex_model(coarse_test_data, return_global_feature=True)
        coarse_face_model_pred_dict, global_feature = coarse_face_model(coarse_test_data, rt_global_features=True)
        coarse_vertices_global_feature = global_feature
        fine_test_data['global_context'] = coarse_vertices_global_feature

        vertex_sample_dict = vertex_model.sample(bsz, context=fine_test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False, adapter_module=decoder_adapter_vertex)
...
def save_ckpts(vertex_model, face_model, cur_step):
    ...
    coarse_vertex_model_sv_folder = os.path.join(sv_ckpts_folder, "coarse_vertex_model")
    os.makedirs(coarse_vertex_model_sv_folder, exist_ok=True)
    coarse_face_model_sv_folder = os.path.join(sv_ckpts_folder, "coarse_face_model")
    os.makedirs(coarse_face_model_sv_folder, exist_ok=True)
    ...
    coarse_vertex_model_sv_pth = os.path.join(coarse_vertex_model_sv_folder, model_sv_path)
    coarse_vertex_model_params = coarse_vertex_model.state_dict()
    torch.save(coarse_vertex_model_params, coarse_vertex_model_sv_pth)

    # model_sv_path = f'{cur_step}.pth'
    coarse_face_model_sv_pth = os.path.join(coarse_face_model_sv_folder, model_sv_path)
    coarse_face_model_params = coarse_face_model.state_dict()
    torch.save(coarse_face_model_params, coarse_face_model_sv_pth) # face model path...
    ...
    print(f"coarse vertex model at step {cur_step} saved to {coarse_vertex_model_sv_pth}.")
    print(f"coarse face model at step {cur_step} saved to {coarse_face_model_sv_pth}.")
...
def resume_from_ckpts():
    ...
    vertex_model.load_state_dict(fine_vertex_model_state_dicts)
    face_model.load_state_dict(fine_face_model_state_dicts)
    coarse_vertex_model.load_state_dict(vertex_model_state_dicts)
    coarse_face_model.load_state_dict(face_model_state_dicts)
    print(f"Vertex model loaded from {fine_vertex_model_path}.")
    print(f"Face model loaded from {fine_face_model_path}.")
    print(f"Coarse vertex model loaded from {vertex_model_path}.")
    print(f"Coarse face model loaded from {face_model_path}.")
...
if __name__=='__main__':
    ...
    # shuffle = False
    # dataet for train...
    dataset_test = torch.utils.data.DataLoader(dataset_test, batch_size=1, shuffle=False, num_workers=num_thread)


    ##### Model configs #####
    vertex_module_config, face_module_config = get_model_configs()
    coarse_vertex_module_config, coarse_face_module_config = get_model_configs(use_light_weight=True)
    # coarse_vertex_module_config, coarse_face_module_config = get_model_configs()
    vertex_model = VertexModel(**vertex_module_config)
    face_model = FaceModel(**face_module_config)

    coarse_vertex_model = VertexModel_np(**coarse_vertex_module_config)
    coarse_face_model = FaceModel_np(**coarse_face_module_config)
    ...
    if len(vertex_model_path) > 0 and len(face_model_path) > 0 and len(fine_vertex_model_path) > 0 and len(fine_face_model_path) > 0:
        resume_from_ckpts()
    ...
    # coarse_encoder_adapter = PrefixKeyValues(**prefix_key_value_config)
    # coarse_decoder_adapter_vertex = PrefixKeyValues(**prefix_key_value_config) # adapter ver
    # coarse_decoder_adapter_face = PrefixKeyValues(**prefix_key_value_config)

    vertex_model = vertex_model.cuda()
    face_model = face_model.cuda()

    coarse_vertex_model = coarse_vertex_model.cuda()
    coarse_face_model = coarse_face_model.cuda()
    ...
    # coarse_encoder_adapter = coarse_encoder_adapter.cuda()
    # coarse_decoder_adapter_vertex = coarse_decoder_adapter_vertex.cuda()
    # coarse_decoder_adapter_face = coarse_decoder_adapter_face.cuda()
    ...
    coarse_vertex_optimizer = optim.Adam(coarse_vertex_model.parameters(), lr=learning_rate)
    coarse_face_optimizer = optim.Adam(coarse_face_model.parameters(), lr=learning_rate)
    ...
    # coarse_encoder_adapter_optimizer = optim.Adam(coarse_encoder_adapter.parameters(), lr=learning_rate)
    # coarse_decoder_adapter_vertex_optimizer = optim.Adam(coarse_decoder_adapter_vertex.parameters(), lr=learning_rate)
    # coarse_decoder_adapter_face_optimizer = optim.Adam(coarse_decoder_adapter_face.parameters(), lr=learning_rate)
    ...
    # train iters... # coarse-to-fine and only use coarse p
