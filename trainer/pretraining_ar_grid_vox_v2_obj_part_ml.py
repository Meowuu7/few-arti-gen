...
from datasets.dataset_pretraining_ar_grid_vox_obj_part_ml import URDFDataset as URDFDataset_ar_obj
...
from modules.modules_pretraining_grid_v2_obj_part_ml import VertexModel
...
def optimize(data, vertex_model, vertex_optimizer):
    ...
    grid_xyz_prediction_loss = -torch.sum( # ientifiers
        pred_dist_grid_xyz.log_prob(data['grid_xyzs']) * data['mask_grid_indicators'].unsqueeze(-1), dim=-1
    ) 
    ...
    grid_values_prediction_loss = -pred_dist_grid_values.log_prob(data['grid_content_vocab']) * data['mask_grid_indicators'] 
...
def step_sample(test_dataset, vertex_model, test_dataset_iter):
...
def sample(test_data, vertex_model, cur_step=0):
    ...
    with torch.no_grad():
        ...
        vertex_sample_dict = vertex_model.sample(bsz, context=test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False, sampling_max_num_grids=sampling_max_num_grids)
        ...
        # masked_grid_xyzs; masked_grid_content_vocab
        test_data_np['grid_xyzs'] = test_data_np['masked_grid_xyzs'] - 1
        test_data_np['grid_values'] = test_data_np['masked_grid_content_vocab']
        context_data_sv_fn = f'training_step_{cur_step}_context.obj'
        ###### plot grids for pretraining ######
        data_utils.plot_grids_for_pretraining_obj_part(v_sample=test_data_np, sv_mesh_folder=sv_mesh_folder, sv_mesh_fn=context_data_sv_fn, cur_step=cur_step) 
...
def train_iter(train_dataset, test_dataset, vertex_model, vertex_optimizer, training_steps, batch_size):
    ...
    for i in range(training_steps):
        ...
        if i % check_step == 0: # then sample from the model and save
            ...
            test_dataset_iter = step_sample(test_dataset, vertex_model, test_dataset_iter)
...
if __name__=='__main__':
    ...
    if opt.dataset.ar_object:
        print(f"Using auto-regressive for objects with parts...")
        URDFDataset = URDFDataset_ar_obj
    else:
        # if not cus_perm_xl:
        if opt.dataset.num_parts > 1:
            print(f"Using multi-parts dataset with number of parts: {opt.dataset.num_parts}.")
    ...
    vertex_model_path = opt.model.vertex_model_path # j
