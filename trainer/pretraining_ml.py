...
# from datasets.urdf_dataset_pretraining_xl import URDFDataset as URDFDataset_xl
from datasets.urdf_dataset_pretraining_ml import URDFDataset as URDFDataset_ar
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_ml_{exp_flag}_fr_{use_eyeglasses_frame}_indi_{category_part_cat}_ctx_{context_window}_face_cond_{face_class_conditional}_light_{use_light_weight}_spec_{specify_light_weight}_vpred_{nn_vertices_predict_ratio}_scale_{random_scaling}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_faces_{nn_max_faces}_max_permit_verts_{nn_max_permite_vertices}_faces_{nn_max_permite_faces}"
...
if __name__=='__main__':
    ...
    dataset_val = torch.utils.data.DataLoader(dataset_val, batch_size=batch_size, shuffle=False, num_workers=num_thread)
    ...
    dataset_test_category_part_indicator_to_idxes = dataset_test.category_part_indicator_to_idxes # dataset test...
    ...
    # train iters...
    train_iter(dataset_train, dataset_val, vertex_model, face_model, vertex_optimizer, face_optimizer, training_steps, batch_size)
