...
def step_sample(vertex_model, cur_iter=0):
    ...
    #     # get data for the next iteration
    #     data = next(test_dataset_iter)
    sample(vertex_model, cur_step=cur_iter)
    return


def sample(vertex_model, cur_step=0):
...
def test_iter(vertex_model, training_steps):
    # dataset_iter
    # iter dataset
    # dataset_iter = iter(test_dataset)
    # test_dataset_iter = iter(test_dataset)
    for i in range(training_steps): # training_steps; 
        step_sample(vertex_model, cur_iter=i) # for the i-th iter sampling
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_ar_vox_prompt_{exp_flag}_indi_{category_part_cat}_ctx_{context_window}_gs_{max_num_grids}_class_{sample_class_idx}_scale_{random_scaling}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_max_permit_verts_{nn_max_permite_vertices}_key_len_{prefix_key_len}"
...
if __name__=='__main__':
    ...
    test_iter(vertex_model, training_steps)
