...
from modules.modules_pretraining_ar_coarse_to_fine_bak import VertexModel, FaceModel
...
def optimize(data, vertex_model, face_model, vertex_optimizer, face_optimizer):
    ...
    fine_vertex_model_pred_dict = vertex_model(fine_data)

    #
    # coarse_face_model_pred_dict = face_model(coarse_data)
    fine_face_model_pred_dict = face_model(fine_data) # fine_face_model
...
def sample(test_data, vertex_model, face_model, cur_step=0):
    ...
    with torch.no_grad():
        ...
        vertex_sample_dict = vertex_model.sample(bsz, context=fine_test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False)
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_ar_ctof_v2_{exp_flag}_cat_part_indi_{category_part_cat}light_{use_light_weight}_shift_{apply_random_shift}_filp_{apply_random_flipping}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_faces_{nn_max_faces}_max_permit_verts_{nn_max_permite_vertices}"
...
if __name__=='__main__':
    ...
    coarse_vertex_model = VertexModel(**coarse_vertex_module_config)
    coarse_face_model = FaceModel(**coarse_face_module_config)
