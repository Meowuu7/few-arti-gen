...
# from datasets.urdf_dataset_pretraining_xl import URDFDataset as URDFDataset_xl
from datasets.urdf_dataset_pretraining_ar_grid_vox_v2_ml import URDFDataset as URDFDataset_ar
...
from modules.modules_pretraining_grid_v2_ml import VertexModel
...
def optimize(data, vertex_model, vertex_optimizer):
    ...
    if pred_dist_grid_values is not None:
        ...
        ##### grid_content_vocab: bsz x seq_length; grid_mask_indicator: bsz x seq_length #####
        ...
        grid_values_prediction_loss = torch.zeros_like(grid_xyz_prediction_loss)
...
def sample(test_data, vertex_model, cur_step=0):
    ...
    with torch.no_grad():
        ...
        if _use_multi_gpu:
          vertex_sample_func = vertex_model.module.sample 
        else:
          vertex_sample_func = vertex_model.sample
        ...
        vertex_sample_dict = vertex_sample_func(bsz, context=test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False)
        ...
        data_utils.plot_grids_for_pretraining_ml(v_sample=vertex_sample_dict, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step, context=test_data_np)
...
if __name__=='__main__':
    ...
    root_dataset_path = os.path.join(dataset_root_path, motion_dataset_nm)
    ...
    # #### for train dataset #### # bucket lighter "faucet", "globe",  "closestool"
     # "wine_bottle", "clock", "cabinet", "lamp", "oven", "seesaw", "washing_machine"
      ...
      category_name = os.listdir(root_dataset_path)
      category_name = [fn for fn in category_name if os.path.isdir(os.path.join(root_dataset_path, fn))]
