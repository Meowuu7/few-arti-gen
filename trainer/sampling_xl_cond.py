...
def sample(test_data, cur_step=0):
    ...
    with torch.no_grad():
        ...
        data_utils.plot_sampled_meshes_single_part_for_pretraining(v_sample=vertex_sample_dict, f_sample=face_sample_dict, context=test_data_np, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step, predict_joint=False)


def test_iter(test_dataset, training_steps, batch_size):
    # dataset_iter
    # iter dataset
...
def resume_from_ckpts(vertex_model_path, face_model_path, pc_encoder_model_path):
    ...
    print(f"PC Encoder model loaded from {pc_encoder_model_path}.")
...
if __name__=='__main__':
    ...
    if motion_cat == "eyeglasses":
      ...
      # test_ins_nns = ["%.4d" % iii for iii in [1, 2, 3, 9, 4, 21]]
      # test_ins_nns = [ "%.4d" % iii for iii in [3, 9, 4, 21]]
    ...
    # resume_from_ckpt
    resume_from_ckpts(vertex_model_path, face_model_path, pc_encoder_model_path)
    ...
    # train iters...
    test_iter(dataset_test, training_steps, batch_size)
