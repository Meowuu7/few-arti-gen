...
# from datasets.urdf_dataset_pretraining_xl import URDFDataset as URDFDataset_xl
from datasets.urdf_dataset_pretraining_ar_grid_vox_ml import URDFDataset as URDFDataset_ar
...
from modules.modules_pretraining_ml import VertexModel, FaceModel
...
def optimize(data, vertex_model, vertex_optimizer):
    ...
    # vertices loss and face loss
    vertex_prediction_loss = -torch.sum( # ientifiers
        vertex_model_pred_dict.log_prob(data['vertices_flat']) * data['mask_vertices_flat_indicator'], dim=-1
    ) 
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_ar_grid_{exp_flag}_fr_{use_eyeglasses_frame}_indi_{category_part_cat}_ctx_{context_window}_vpred_{nn_vertices_predict_ratio}_scale_{random_scaling}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_max_permit_verts_{nn_max_permite_vertices}"
...
if __name__=='__main__':
    ...
    dataset_val = URDFDataset(root_folder=root_folder, quantization_bits=quantization_bits, nn_max_vertices=nn_max_vertices, nn_max_faces=nn_max_faces, nn_max_permite_vertices=nn_max_permite_vertices, nn_max_permite_faces=nn_max_permite_faces, category_name=category_names, instance_nns=train_ins_nns, part_names=None, category_nm_to_part_nm=category_names_to_part_names, is_training=True, split="validation")

    # dataset_train_category_part_indicator_to_idxes = dataset_train.category_part_indicator_to_idxes

    dataset_val = torch.utils.data.DataLoader(dataset_val, batch_size=batch_size, shuffle=True, num_workers=num_thread)
    ...
    # if len(vertex_model_path) > 0 and len(face_model_path) > 0:
    ...
    train_iter(dataset_train, dataset_val, vertex_model, vertex_optimizer, training_steps, batch_size)
