...
from datasets.urdf_dataset_pretraining_xl_two_parts import URDFDataset as URDFDataset_xl
...
from modules.modules_pretraining_xl_two_parts import VertexModel, FaceModel
...
def optimize(data, vertex_model, face_model, vertex_optimizer, face_optimizer):
    ...
    for k in data:
        if isinstance(data[k], dict):
          for sub_k in data[k]:
            data[k][sub_k] = data[k][sub_k].cuda(non_blocking=True)
        else:
          data[k] = data[k].cuda(non_blocking=True)
        # print(f"key: {k}, shape: {data[k].size()}")

    # vertex_model_pred_dict
    vertex_model_pred_dict, vertex_model_pred_dict_rgt = vertex_model(data)
    # TODO: construct face data and 
    face_model_pred_dict = face_model(data)
    face_model_pred_dict_rgt = face_model(data['rgt'])
    # face_model_pred_dict_rgt = facemodel
    ...
    vertex_prediction_loss_rgt = -torch.sum( # ientifiers
        vertex_model_pred_dict_rgt.log_prob(data['vertices_flat_rgt']) * data['vertices_flat_mask_rgt'], dim=-1
    ) 

    vertex_prediction_loss_rgt = torch.sum(vertex_prediction_loss_rgt) #

    vertex_prediction_loss = vertex_prediction_loss + vertex_prediction_loss_rgt
    ...
    # face prediction loss
    face_prediction_loss_rgt = -torch.sum(
        face_model_pred_dict_rgt.log_prob(data['rgt']['faces']) * data['rgt']['faces_mask'], dim=-1
    ) 

    face_prediction_loss_rgt = torch.sum(face_prediction_loss_rgt)

    face_prediction_loss = face_prediction_loss + face_prediction_loss_rgt
...
def sample(test_data, vertex_model, face_model, cur_step=0):
    ...
    with torch.no_grad():
        ...
        for k in test_data:
          ...
          if isinstance(test_data[k], dict):
            for sub_k in test_data[k]:
              test_data[k][sub_k] = test_data[k][sub_k].cuda(non_blocking=True)
          else:
            test_data[k] = test_data[k].cuda(non_blocking=True)

        vertex_sample_dict, vertex_sample_dict_rgt = vertex_model.sample(
            bsz, context=test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=False, only_return_complete=False
        )


        face_sample_dict = face_model.sample(
            vertex_sample_dict, max_sample_length=nn_max_faces, temperature=1., top_k=0,  top_p=1.0,  only_return_complete=False
        )

        face_sample_dict_rgt = face_model.sample(
            vertex_sample_dict_rgt, max_sample_length=nn_max_faces, temperature=1., top_k=0,  top_p=1.0,  only_return_complete=False
        )
        ...
        for k in vertex_sample_dict_rgt: # sample dict
            if isinstance(vertex_sample_dict_rgt[k], dict):
                for sub_k in vertex_sample_dict_rgt[k]:
                    vertex_sample_dict_rgt[k][sub_k] = vertex_sample_dict_rgt[k][sub_k].detach().cpu().numpy()
            else:
                vertex_sample_dict_rgt[k] = vertex_sample_dict_rgt[k].detach().cpu().numpy()
        for k in face_sample_dict_rgt:
            if isinstance(face_sample_dict_rgt[k], dict):
                for sub_k in face_sample_dict_rgt[k]:
                    face_sample_dict_rgt[k][sub_k] = face_sample_dict_rgt[k][sub_k].detach().cpu().numpy()
            else:
                face_sample_dict_rgt[k] = face_sample_dict_rgt[k].detach().cpu().numpy()
        test_data_np = {} # numpys
        for k in test_data:
            if isinstance(test_data[k], dict):
                test_data_np[k] = {}
        ...
        data_utils.plot_sampled_meshes_single_part_for_pretraining(v_sample=vertex_sample_dict, f_sample=face_sample_dict, context=test_data_np, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step * 2, predict_joint=False) # predict joint

        data_utils.plot_sampled_meshes_single_part_for_pretraining(v_sample=vertex_sample_dict_rgt, f_sample=face_sample_dict_rgt, context=test_data_np, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step * 2 + 1, predict_joint=False) # predict joint
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_pure_xl_{exp_flag}_tot_cus_{cus_perm_xl}_cross_{cross}_4s_hift_{apply_random_shift}_filp_{apply_random_flipping}_{motion_dataset_nm}_cat_{motion_cat}_bsz_{batch_size}_max_verts_{nn_max_vertices}_faces_{nn_max_faces}_max_permit_verts_{nn_max_permite_vertices}_faces_{nn_max_permite_faces}_mask_low_{mask_low_ratio}_high_{mask_high_ratio}"
...
if __name__=='__main__':
    ...
    # #### for train dataset ####
    # # category_name = ["eyeglasses", "pen", "scissors" ] # pen, scissors, screwdriver, seesaw, stapler
