...
def sample(test_data,  cur_step=0):
    ...
    with torch.no_grad():
        ...
        lft_vertex_model_pred_dict, global_feature = vertex_model(coarse_test_data, return_global_feature=True)
        fine_test_data['global_context'] = global_feature

        # have no global context here...
        # lft_vertex_sample_dict = vertex_mo/del.sample(bsz, context=coarse_test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False)
...
def test_iter(test_dataset, training_steps):
    ...
    for i in range(training_steps): # training_steps; 
        ...
        test_dataset_iter = step_sample(test_dataset, test_dataset_iter, cur_step=i) # for the i-th iter sampling
