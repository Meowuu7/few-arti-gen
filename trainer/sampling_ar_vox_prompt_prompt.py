...
def step_sample(test_dataset, vertex_model, adapter_modules, test_dataset_iter, cur_iter=0):
    ...
    try:
        ...
        data = next(test_dataset_iter)
    sample(data, vertex_model, adapter_modules, cur_step=cur_iter)
...
def sample(test_data, vertex_model, adapter_modules, cur_step=0):
    ...
    with torch.no_grad():
        ...
        # class_label 
        sample_context = {
            'class_label': torch.full(size=(bsz, 1), fill_value=class_label, dtype=torch.long).cuda()
        }
        # perform vertex sampling #
        vertex_sample_dict = vertex_model.sample(bsz, context=sample_context, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False, adapter_modules=adapter_modules)
...
# def step_sample(test_dataset, vertex_model, adapter_modules, test_dataset_iter, cur_iter=0):
def test_iter(test_dataset, vertex_model, adapter_modules, training_steps):
    # dataset_iter
    # iter dataset
    # dataset_iter = iter(test_dataset)
    test_dataset_iter = iter(test_dataset)
    for i in range(training_steps): # training_steps;
        step_sample(test_dataset, vertex_model, adapter_modules, test_dataset_iter, cur_iter=i) # for the i-th iter sampling
...
def get_model_descriptor():
    ...
    class_label = opt.model.sample_class_idx
    model_descriptor = f"{exp_mode}_ar_vox_prompt_{exp_flag}_indi_{category_part_cat}_cls_{class_label}_ctx_{context_window}_gs_{max_num_grids}_scale_{random_scaling}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_max_permit_verts_{nn_max_permite_vertices}_key_len_{prefix_key_len}"
...
def resume_from_ckpts(vertex_model, adapter_modules, vertex_model_path, encoder_adapter_path):
    ...
    adapter_modules_state_dicts = torch.load(encoder_adapter_path, map_location='cpu')


    safe_load_ckpt_common(vertex_model, vertex_model_state_dicts)
    # safe_load_ckpt_common(face_model, face_model_state_dicts)
    safe_load_ckpt_common(adapter_modules, adapter_modules_state_dicts)
    ...
    print(f"Adapter modules loaded from {encoder_adapter_path}.")
...
if __name__=='__main__':
    ...
    encoder_adapter_path = opt.model.encoder_adapter_path

    if len(vertex_model_path) > 0 and len(encoder_adapter_path) > 0:
        resume_from_ckpts(vertex_model, adapter_modules, vertex_model_path, encoder_adapter_path)
    ...
    training_steps = opt.loss.training_steps # --training-steps=xxx
    ...
    test_iter(dataset_test, vertex_model, adapter_modules, training_steps)
