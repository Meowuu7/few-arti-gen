...
from datasets.urdf_dataset_pretraining_ar_two_parts import URDFDataset as URDFDataset_ar
...
from modules.modules_pretraining_ar_two_parts import VertexModel, FaceModel
...
def optimize(data, vertex_model, face_model, vertex_optimizer, face_optimizer):
    ...
    coarse_data, fine_data = data['lft'], data['rgt']
    ...
    fine_face_model_pred_dict = fine_face_model(fine_data)
    ...
    if step_counter % check_step == 0:
      ...
      print(f"Left vertex Loss {coarse_vertex_prediction_loss.sum().item()}, Right vertex loss: {fine_vertex_prediction_loss.sum().item()}")
      print(f"Left face Loss {coarse_face_prediction_loss.sum().item()}, Right face loss: {fine_face_prediction_loss.sum().item()}")
...
def sample(test_data, vertex_model, face_model, cur_step=0):
    ...
    bsz = min(bsz, test_data['lft']['vertices'].size(0))
    ##### Get vertices and faces here #####
    ...
    with torch.no_grad():
        ...
        coarse_test_data, fine_test_data = test_data['lft'], test_data['rgt']
        ...
        # have no global context here...
        lft_vertex_sample_dict = vertex_model.sample(bsz, context=coarse_test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False)

        lft_vertices_global_feature = lft_vertex_sample_dict['vertices_global_feature']
        fine_test_data['global_context'] = lft_vertices_global_feature
        ...
        lft_face_sample_dict = face_model.sample(lft_vertex_sample_dict, max_sample_length=nn_max_faces, temperature=1., top_k=0,  top_p=1.0,  only_return_complete=False)
        ...
        data_utils.plot_sampled_meshes_single_part_for_pretraining(v_sample=lft_vertex_sample_dict, f_sample=lft_face_sample_dict, context=test_data_np, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step * 2, predict_joint=False) # predict joint variable...

        data_utils.plot_sampled_meshes_single_part_for_pretraining(v_sample=vertex_sample_dict, f_sample=face_sample_dict, context=test_data_np, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step * 2 + 1, predict_joint=False) # predict joint variable...
        # except:
        #     pass
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_ar_two_parts_{exp_flag}_cat_part_indi_{category_part_cat}_shift_{apply_random_shift}_filp_{apply_random_flipping}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_faces_{nn_max_faces}_max_permit_verts_{nn_max_permite_vertices}_faces_{nn_max_permite_faces}"
...
if __name__=='__main__':
    ...
    # #### for train dataset ####
      ...
      # category_name = ["eyeglasses", ]
      # category_names_to_part_names = {
      #     "eyeglasses": ["dof_rootd_Aa001_r", "dof_rootd_Aa002_r"],  # dofs
      #     # "pen": None, "scissors": None, 
      # }
      # category_names = list(category_names_to_part_names.keys())
      # # #### for train dataset #### # (right part | left part) conditional

      category_name = ["screwdriver", "closestool", "eyeglasses",  "pen", "scissors", "stapler"]
      category_names_to_part_names = {cat: None for cat in category_name}
