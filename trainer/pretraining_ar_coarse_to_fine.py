...
def optimize(data, vertex_model, face_model, vertex_optimizer, face_optimizer):
    ...
    vertex_model_pred_dict = vertex_model(data, return_global_feature=True)
    face_model_pred_dict, global_feature = face_model(data, rt_global_features=True)
    ...
    if step_counter % check_step == 0:
      ...
      print(f"Coarse vertex Loss {vertex_prediction_loss.sum().item()}")
      print(f"Coarse face Loss {face_prediction_loss.sum().item()}")
...
def sample(test_data, vertex_model, face_model, cur_step=0):
    ...
    bsz = min(bsz, test_data['vertices'].size(0))
    ##### Get vertices and faces here #####
    ...
    with torch.no_grad():
        ...
        test_data_np = {} # numpys
        for k in test_data:
            test_data_np['vertices'] = test_data['coarse_vertices'].detach().cpu().numpy()
            test_data_np['faces'] = test_data['coarse_faces'].detach().cpu().numpy()
            test_data_np['vertices_mask'] = test_data['coarse_vertices_mask'].detach().cpu().numpy()
            test_data_np['num_face_indices'] = test_data['coarse_num_face_indices'].detach().cpu().numpy()
            test_data_np['nn_faces'] = test_data['coarse_num_face_indices'].detach().cpu().numpy()
            test_data_np['faces_mask'] = test_data['coarse_faces_mask'].detach().cpu().numpy()
