...
from datasets.urdf_datasset_pretraining_xl_cond import URDFDataset as URDFDataset_xl
...
def optimize(data):
    ...
    # print(f"context_feature: {context_feature.size()}")
    data['global_context'] = context_feature
...
# the context vertices distance and the sampled vertices distance?
...
def sample(test_data, cur_step=0):
    ...
    with torch.no_grad():
        ...
        test_data['global_context'] = context_feature

        bsz = sampled_pts.size(0)
...
if __name__=='__main__':
    ...
    pc_encoder_cond_model = PointnetPP(in_feat_dim=3)
