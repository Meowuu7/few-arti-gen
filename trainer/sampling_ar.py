...
def sample(test_data, vertex_model, face_model, cur_step=0):
    ...
    beam_search = opt.model.beam_search
    ...
    with torch.no_grad():
        ...
        print(f"Sampling classss label: {sample_class_idx}") # sample_class_idx
        sample_context = {
            'class_label': torch.full(size=(bsz,), fill_value=sample_class_idx, dtype=torch.long).cuda()
        }
        # perform vertex sampling # sample vertices
        # temperature = 7.0
        # temperature = 4.0
        # temperature = 2.0
        temperature = 1.0
        if not beam_search:
            vertex_sample_dict = vertex_model.sample(bsz, context=sample_context, max_sample_length=nn_max_vertices, temperature=temperature, top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False)
        else:
            vertex_sample_dict = vertex_model.sample_beam_search(bsz, context=sample_context, max_sample_length=nn_max_vertices, temperature=temperature, top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False)
        ...
        # data_utils.plot_sampled_meshes_single_part_for_pretraining(v_sample=vertex_sample_dict, f_sample=face_sample_dict, context=test_data_np, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step, predict_joint=False, with_context=False) # predict joint variable...
        # plot sampled mesh...
        data_utils.plot_sampled_meshes_single_part_for_sampling(v_sample=vertex_sample_dict, f_sample=face_sample_dict, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step, predict_joint=False)
        
        cur_vertices_probs = vertex_sample_dict['probs']
        # cur_vertices_probs = cur_vertices_probs.detach().cpu().numpy()
        for i_bsz in range(cur_vertices_probs.shape[0]):
            cur_bsz_vertices_probs = cur_vertices_probs[i_bsz]
            cur_bsz_vertices_probs_sv_fn = os.path.join(sv_mesh_folder, f"sample_probs_step_{cur_step}_ins_{i_bsz}.npy")
            np.save(cur_bsz_vertices_probs_sv_fn, cur_bsz_vertices_probs) # save probs here




# train iter
def test_iter(test_dataset, vertex_model, face_model, training_steps, batch_size):
    # dataset_iter
    # iter dataset
    ...
    for i in range(training_steps): # training_steps; 
        ...
        test_dataset_iter = step_sample(test_dataset, vertex_model, face_model, test_dataset_iter, cur_iter=i) # for the i-th iter sampling
...
def get_model_descriptor():
    ...
    beam_search = opt.model.beam_search
    model_descriptor = f"sampling_{exp_flag}_beam_{beam_search}_ar_part_indi_{category_part_cat}_{motion_dataset_nm}_cat_{motion_cat}_bsz_{batch_size}_max_verts_{nn_max_vertices}_faces_{nn_max_faces}_max_permit_verts_{nn_max_permite_vertices}_faces_{nn_max_permite_faces}_mask_low_{mask_low_ratio}_high_{mask_high_ratio}"
...
if __name__=='__main__':
    ...
    # nn_max_vertices = 400
    # nn_max_faces = 2000
    nn_max_vertices = opt.vertex_model.max_vertices # vertex_model
    nn_max_faces = opt.face_model.max_faces
    ...
    #### for train dataset ####
    category_name = ["eyeglasses"] # eyeglasses
    category_name = ["subd0"] # eyeglasses
    # category_names_to_part_names = {
        # "eyeglasses": ["none_motion"]
    # }
    ...
    # load eyeglasses dataset
    dataset_test = URDFDataset(root_folder=root_folder, quantization_bits=quantization_bits, nn_max_vertices=nn_max_vertices, nn_max_faces=nn_max_faces, nn_max_permite_vertices=nn_max_permite_vertices, nn_max_permite_faces=nn_max_permite_faces, category_name=category_names, instance_nns=test_ins_nns, part_names=None, category_nm_to_part_nm=category_names_to_part_names)
    ...
    # train iters...
    test_iter(dataset_test, vertex_model, face_model, training_steps, batch_size)
