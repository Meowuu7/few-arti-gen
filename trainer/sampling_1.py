...
def get_model_descriptor():
    ...
    # cross = opt.model.cross
    model_descriptor = f"sampling_cross_{cross}_{motion_dataset_nm}_cat_{motion_cat}_bsz_{batch_size}_max_verts_{nn_max_vertices}_faces_{nn_max_faces}_max_permit_verts_{nn_max_permite_vertices}_faces_{nn_max_permite_faces}_mask_low_{mask_low_ratio}_high_{mask_high_ratio}_mask_{mask_vertices_type}"
...
if __name__=='__main__':
    ...
    if opt.dataset.lab == "thu": # save samples
        sv_mesh_folder = os.path.join("/nas/datasets/gen", "sampling_samples")
        ...
        os.makedirs("./sampling_samples", exist_ok=True)
        os.makedirs(sv_mesh_folder, exist_ok=True)
