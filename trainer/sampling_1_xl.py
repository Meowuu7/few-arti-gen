...
# from datasets.urdf_dataset_paired_finetuning_cus_perm_xl import URDFDataset as URDFDataset_cus_perm_xl
...
from modules.modules_pretraining_xl_2 import VertexModel, FaceModel # models
...
def sample(test_dataset, vertex_model, face_model, sv_samples_folder, cur_step=0):
    ...
    with torch.no_grad():
        for test_data in test_dataset:
            ...
            # vertices of the current data... faces of the current data...
            # max_sample_length?
            bsz = test_data['vertices_flat'].size(0)
            for i_bsz in range(bsz):
              print(f"Sample for {i_bsz_ori}-th eyeglasses sample.") # eyeglasses
              cur_bsz_item_idx = int(test_data['cur_data_idx'][i_bsz].item())
              cur_bsz_mesh_dict = test_dataset.dataset.mesh_dicts[i_bsz]
              cur_test_data_dict = {}
              for k in test_data:
                cur_k_cur_bsz_data = test_data[k][i_bsz].unsqueeze(0)
                cur_k_cur_bsz_data = torch.cat([cur_k_cur_bsz_data for _ in range(n_samples)], dim=0)
                cur_test_data_dict[k] = cur_k_cur_bsz_data
              vertex_sample_dict = vertex_model.sample(bsz, context=cur_test_data_dict, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False)
              ...
              cur_bsz_backbone_verts = cur_bsz_mesh_dict['vertices']
              cur_bsz_backbone_verts = data_utils.dequantize_verts(cur_bsz_backbone_verts, 8)
              cur_bsz_backbone_faces = data_utils.unflatten_faces(cur_bsz_mesh_dict['faces'])
              
              cur_bsz_sv_folder = os.path.join(sv_samples_folder, f"mesh-{i_bsz_ori}")
              i_bsz_ori += 1
              os.makedirs(cur_bsz_sv_folder, exist_ok=True)
              cur_bsz_backbone_sv_fn = os.path.join(cur_bsz_sv_folder, "backbone.obj")
              data_utils.write_obj(cur_bsz_backbone_verts, cur_bsz_backbone_faces, cur_bsz_backbone_sv_fn, transpose=True, scale=1.)
              ...
              data_utils.plot_sampled_meshes_single_part_for_sampling(v_sample=vertex_sample_dict, f_sample=face_sample_dict, sv_mesh_folder=cur_bsz_sv_folder, cur_step=cur_step, predict_joint=predict_joint)
...
if __name__=='__main__':
    ...
    if motion_cat == "eyeglasses":
      ...
      test_ins_nns = train_ins_nns + test_ins_nns
    ...
    sv_samples_folder = os.path.join(dataset_root_path, motion_dataset_nm + "_Sampled")
    os.makedirs(sv_samples_folder, exist_ok=True)
    sv_samples_folder = os.path.join(sv_samples_folder, opt.dataset.category)
    os.makedirs(sv_samples_folder, exist_ok=True)

    sample(dataset_test, vertex_model, face_model, sv_samples_folder, cur_step=0)
# sampling...