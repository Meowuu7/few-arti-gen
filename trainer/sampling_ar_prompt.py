...
def resume_from_ckpts(vertex_model, face_model, vertex_model_path, face_model_path):
    ...
    encoder_adapter_state_dicts = torch.load(encoder_adapter_path, map_location="cpu")
    decoder_adapter_face_state_dicts = torch.load(decoder_adapter_face_path, map_location="cpu")
    decoder_adapter_vertex_state_dicts = torch.load(decoder_adapter_vertex_path, map_location="cpu")
    

    vertex_model.load_state_dict(vertex_model_state_dicts)
    face_model.load_state_dict(face_model_state_dicts)
    encoder_adapter.load_state_dict(encoder_adapter_state_dicts)
    decoder_adapter_face.load_state_dict(decoder_adapter_face_state_dicts)
    decoder_adapter_vertex.load_state_dict(decoder_adapter_vertex_state_dicts)
...
if __name__=='__main__':
    ...
    decoder_adapter_face_path = opt.model.decoder_adapter_face_path
    decoder_adapter_vertex_path = opt.model.decoder_adapter_vertex_path

    if len(vertex_model_path) > 0 and len(face_model_path) > 0 and len(encoder_adapter_path) > 0 and len(decoder_adapter_face_path) > 0 and len(decoder_adapter_vertex_path) > 0:
