...
from modules.modules_pretraining_vox import VertexModel
...
def sample(test_data, vertex_model, cur_step=0):
    ...
    vertex_model.eval()
    # face_model.eval()
    class_label = opt.model.sample_class_idx # class labels
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_ar_grid_{exp_flag}_indi_{category_part_cat}_ctx_{context_window}_gs_{max_num_grids}_scale_{random_scaling}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_max_permit_verts_{nn_max_permite_vertices}"
