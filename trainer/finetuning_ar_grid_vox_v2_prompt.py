...
def optimize_with_adaptation(data, vertex_model, adapter_modules, adapter_modules_optimizer):
    # global use_cuda
    global step_counter
    ...
    # face_model_pred_dict = face_model(data)
    loss_ori_grid_xyzs, loss_maxx_grid_xyzs, grid_values_prediction_loss = vertex_model.forward_with_adaptation(data, adapter_modules=adapter_modules)
    ...
    prediction_loss = torch.mean(grid_values_prediction_loss + (loss_maxx_grid_xyzs + loss_ori_grid_xyzs) / 2.)
    ...
    if step_counter % check_step == 0:
      print(f"Step {step_counter}")
      print(f"Vertex Loss {prediction_loss.item()}, grid_ori_xyz_prediction_loss: {loss_ori_grid_xyzs.mean().item()}, grid_maxx_xyz_prediction_loss: {loss_maxx_grid_xyzs.mean().item()}, grid_values_prediction_loss: {grid_values_prediction_loss.mean().item()}")
      # print(f"Face Loss {face_prediction_loss.item()}")
    step_counter += 1
...
def calculate_loss(data, vertex_model, adapter_modules, adapter_modules_optimizer):
    # global use_cuda
    # global step_counter
    ...
    with torch.no_grad():
        ...
        # grid_xyz_prediction_loss: bsz x grid_length x 3 # grid_xyz_prediction_loss
        grid_xyz_prediction_loss = -torch.sum( # ientifiers
            pred_dist_grid_xyz.log_prob(data['grid_xyzs']) * data['grid_xyzs_mask'], dim=-1
        ) 
        grid_xyz_prediction_loss = grid_xyz_prediction_loss.sum(-1)
...
def step(train_dataset, vertex_model, adapter_modules, adapter_modules_optimizer, batch_size, dataset_iter):
    ...
    try:
        ...
        data = next(dataset_iter)
    adaptation_optimization = opt.model.adaptation_optimization
    if adaptation_optimization:
      optimize_with_adaptation(data, vertex_model, adapter_modules, adapter_modules_optimizer)
    else:
...
def step_calculate_loss(train_dataset, vertex_model, adapter_modules, adapter_modules_optimizer, batch_size, dataset_iter):
    # global epoch_counter
    # try:
    #     data = next(dataset_iter)
    #     if data['class_label'].size(0) < batch_size:
    ...
    for data in train_dataset:
        # adaptation_optimization = opt.model.adaptation_optimization
        cur_loss = calculate_loss(data, vertex_model, adapter_modules, adapter_modules_optimizer)
        tot_loss += cur_loss
        tot_step += 1
    tot_loss = tot_loss / float(tot_step)
...
def sample(test_data, vertex_model, adapter_modules, cur_step=0):
    ...
    with torch.no_grad():
        ...
        vertex_sample_dict = vertex_model.sample(bsz, context=test_data, adapter_modules=adapter_modules, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False, sampling_max_num_grids=sampling_max_num_grids)
        ...
        if multi_parts_not_ar_object:
            ...
            data_utils.plot_grids_for_pretraining_obj_part(v_sample=vertex_sample_dict, sv_mesh_folder=sv_mesh_folder, cur_step=cur_step) 
        else:
...
def train_iter(train_dataset, test_dataset, vertex_model, adapter_modules, adapter_modules_optimizer, training_steps, batch_size):
    ...
    for i in range(training_steps):
        ...
        if i % check_step == 0: # then sample from the model and save
            ...
            val_loss = calculate_loss_iter(test_dataset, test_dataset, vertex_model, adapter_modules, adapter_modules_optimizer,  training_steps, batch_size)
            ...
            if val_loss < minn_val_loss:
                ...
                minn_val_step = i
            print(f"Current validation loss: {val_loss}, minn validation loss: {minn_val_loss}, minn validation step: {minn_val_step}.")



# train iter
def calculate_loss_iter(train_dataset, test_dataset, vertex_model, adapter_modules, adapter_modules_optimizer, training_steps, batch_size):
    # dataset_iter
    # iter dataset
    dataset_iter = iter(train_dataset)
    # test_dataset_iter = iter(test_dataset)
    tot_loss = step_calculate_loss(train_dataset, vertex_model, adapter_modules, adapter_modules_optimizer, batch_size, dataset_iter)
    return tot_loss
...
def get_model_descriptor():
    ...
    model_descriptor = f"{exp_mode}_ar_grid_vocab_{exp_flag}_indi_{category_part_cat}_ctx_{context_window}_gs_{grid_size}_ngrids_{max_num_grids}_scale_{random_scaling}_{motion_dataset_nm}_bsz_{batch_size}_max_verts_{nn_max_vertices}_max_permit_verts_{nn_max_permite_vertices}_key_{prefix_key_len}"
