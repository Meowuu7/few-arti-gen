...
def optimize(data, vertex_model, vertex_optimizer):
    ...
    if step_counter % check_step == 0:
      ...
      print(f"Total prediction loss: {all_pred_loss}, face_loss: {face_prediction_loss.mean().item()}, Total reg_sigma loss: {all_reg_sigma_loss}, Avg loss: {tot_prediction_loss.item()}")
...
def sample(test_data, vertex_model, cur_step=0):
    ...
    with torch.no_grad():
        ...
        sampled_vertices = vertex_model.sample(bsz, context=test_data)

        print(f"Vertex sampling completed...")
        
        sampled_faces = face_model.sample(context=sampled_vertices)
...
if __name__=='__main__':
    ...
    #   category_name = ["data_v0_deci_0.2"]
      category_name = ["eyeglasses"] # ["screwdriver"] # eyeglasses...
