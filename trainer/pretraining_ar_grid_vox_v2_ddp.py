...
def sample(test_data, vertex_model, cur_step=0):
    ...
    with torch.no_grad():
        ...
        vertex_sample_dict = vertex_sample_func(bsz, context=test_data, max_sample_length=nn_max_vertices, temperature=1., top_k=0, top_p=1.0, recenter_verts=True, only_return_complete=False, sampling_max_num_grids=sampling_max_num_grids)
...
if __name__=='__main__':
    ...
    #### only grids can support our multi parts setting? ####
    ...
    if not debug:
      ...
      if (not opt.model.specify_light_weight) and (not opt.dataset.use_eyeglasses_frame) and ("eyeglasses" in category_names_to_part_names): # then wes
    ...
    dataset_test = URDFDataset(root_folder=root_folder, quantization_bits=quantization_bits, nn_max_vertices=nn_max_vertices, nn_max_faces=nn_max_faces, nn_max_permite_vertices=nn_max_permite_vertices, nn_max_permite_faces=nn_max_permite_faces, category_name=category_names, instance_nns=train_ins_nns, part_names=None, category_nm_to_part_nm=category_names_to_part_names, is_training=False, split='validation')
