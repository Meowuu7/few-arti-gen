...
# #### vox prompt ####
# #### parts all trained model without scaling ####
# # grid size; quantization bits...
# CUDA_VISIBLE_DEVICES=1 python trainer/finetuning_ar_vox_prompt_prompt.py  --exp-mode="finetuning"  --max-permit-vertices=800 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64_v9_ns_50" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=800 --max-faces=3000 --batch-size=1  --check-step=200 --exp-flag="parts_v9_frames_pr_v4" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=800 --grid-size=1  --quantization-bits=6 --vertex-model-path="/nas/datasets/gen/ckpts/pretraining_ar_vox_prompt_parts_all_indi_True_ctx_400_gs_800_scale_0_MotionDataset_processed_voxelized_64_bsz_1_max_verts_800_max_permit_verts_800_key_len_512/vertex_model/140000.pth" --prefix-key-len=512 --prefix-value-len=512  --category-part-indicator=True --nn-training-categories=9 --load-meta=True  # --debug=True   # --debug=True #  --apply-random-scaling=1 --context-window=900


#### vox prompt ####
#### parts v9 trained model with scaling ####
# ##### MotionDataset_processed_voxelized_64_v9_ns_50
# grid size; quantization bits...
CUDA_VISIBLE_DEVICES=1 python trainer/finetuning_ar_vox_prompt_prompt.py  --exp-mode="finetuning"  --category="eyeglasses"  --max-permit-vertices=800 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64_ns_50" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=800 --max-faces=3000 --batch-size=1  --check-step=200 --exp-flag="parts_v9_frames_pr_v4" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=800 --grid-size=1  --quantization-bits=6 --vertex-model-path="/nas/datasets/gen/ckpts/pretraining_ar_vox_prompt_parts_v9_4n_indi_True_ctx_400_gs_800_scale_0_MotionDataset_processed_voxelized_64_v9_ns_50_bsz_1_max_verts_800_max_permit_verts_800_key_len_512/vertex_model/98000.pth" --prefix-key-len=512 --prefix-value-len=512  --category-part-indicator=True --nn-training-categories=9 --load-meta=True --context-window=300 # --debug=True   # --debug=True #  --apply-random-scaling=1 --context-window=900

#### finetune, load-meta, random-scaling, context-window
