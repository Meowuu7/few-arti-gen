...
CUDA_VISIBLE_DEVICES=0 python trainer/pretraining_ar_grid.py  --exp-mode="pretraining"  --max-permit-vertices=900 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_part_first" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=900 --max-faces=3000 --batch-size=1  --check-step=2000 --exp-flag="some_parts_v0_2" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=900 --grid-size=4   #  --debug=True # --category-part-indicator=True  # --debug=True
