...
CUDA_VISIBLE_DEVICES=6 python trainer/pretraining_ar_two_parts.py  --exp-mode="pretraining" --use-light-weight=True  --max-permit-vertices=400 --max-permit-faces=2000 --dataset-name="MotionDataset_processed_deci_part_first" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=400 --max-faces=2000 --batch-size=10  --mask-low-ratio=0.30 --mask-high-ratio=0.50  --check-step=200  --coarse-angle-limit=30 --fine-angle-limit=15 --exp-flag="some_parts_v3" --num-classes=177 # --debug=True --category-part-indicator=True
