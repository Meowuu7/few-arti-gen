...
trainer/pretraining_ar_grid_vox_m2_ml_ddp.py  --exp-mode="pretraining"  --max-permit-vertices=1000 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64_ns_50" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=1000 --max-faces=3000 --batch-size=1  --check-step=200 --exp-flag="vox_ml_n6" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=750 --grid-size=1  --quantization-bits=6  --load-meta=True  --prefix-key-len=256 --prefix-value-len=256 --mask-low-ratio=0.15 --mask-high-ratio=0.16 #  --use-inst=True #  --category-part-indicator=True # --debug=True   # --debug=True # --context-window=400
