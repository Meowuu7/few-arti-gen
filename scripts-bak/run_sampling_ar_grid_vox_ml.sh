...
#### MotionDataset_processed

CUDA_VISIBLE_DEVICES=5 python trainer/sampling_ar_grid_vox_ml.py  --exp-mode="sampling"  --max-permit-vertices=1100 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=1100 --max-faces=3000 --batch-size=1  --check-step=2000 --exp-flag="v0_vox_ml_0.70" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --quantization-bits=6   --category-part-indicator=True --mask-low-ratio=0.70 --mask-high-ratio=0.71 --vertex-model-path="/nas/datasets/gen/ckpts/pretraining_ar_grid_v0_vox_ml_0.7_fr_False_indi_True_ctx_-1_vpred_1.0_scale_0_MotionDataset_processed_voxelized_64_bsz_1_max_verts_1100_max_permit_verts_1100/vertex_model/122000.pth" # --data-type='mesh' # --debug=True   # --debug=True
