...
# grid size; quantization bits...
##### grid, context window, gridsize #####
CUDA_VISIBLE_DEVICES=1 python trainer/pretraining_ar_grid_vox.py  --exp-mode="pretraining"  --max-permit-vertices=2000 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=2000 --max-faces=3000 --batch-size=1  --check-step=200 --exp-flag="grid_vox_v1_legs" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=250 --grid-size=2  --quantization-bits=6 --category-part-indicator=True --debug=True   #  --category-part-indicator=True # --debug=True   # --debug=True
