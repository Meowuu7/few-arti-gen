...
for sampling_idx in 51 37 0 3 10 25 46 44 57
do
  # grid size; quantization bits...
  CUDA_VISIBLE_DEVICES=0 python trainer/sampling_ar_vox.py  --exp-mode="sampling"  --max-permit-vertices=900 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=900 --max-faces=3000 --batch-size=1  --check-step=200 --exp-flag="all_parts_nscale" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=900 --grid-size=1  --quantization-bits=6 --debug=True --vertex-model-path="/nas/datasets/gen/ckpts/pretraining_ar_grid_all_parts_fr_False_indi_True_ctx_400_gs_900_vpred_1.0_scale_0_MotionDataset_processed_voxelized_64_bsz_1_max_verts_900_max_permit_verts_900/vertex_model/152000.pth" --sample-class-idx=${sampling_idx} --apply-random-scaling=0  --category-part-indicator=True --training-steps=20 --context-window=400 --ar-object=True # --debug=True  # --context-window=400  # --debug=True # not removing du...
done
