...
######## pretraining ar vox prompt ##########

CUDA_VISIBLE_DEVICES=2 python trainer/pretraining_ar_vox_prompt.py  --exp-mode="pretraining"  --max-permit-vertices=800 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_32_v9_ns_50" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=800 --max-faces=3000 --batch-size=1  --check-step=2000 --exp-flag="parts_frames_n1" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=800 --grid-size=1  --quantization-bits=6  --category-part-indicator=True  --prefix-key-len=512 --prefix-value-len=512 --load-meta=True  --context-window=300 --with-prompt-encoder=True   # --debug=True #  --use-inst=True --apply-random-scaling=1 --apply-random-scaling=1  --context-window=400 
