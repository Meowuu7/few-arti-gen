...
# grid size; quantization bits...
CUDA_VISIBLE_DEVICES=7 python trainer/sampling_ar_grid_vox.py  --exp-mode="sampling"  --max-permit-vertices=2000 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=2000 --max-faces=3000 --batch-size=1  --check-step=2000 --exp-flag="grid_vox" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=250 --grid-size=2  --quantization-bits=6 --category-part-indicator=True --debug=True --vertex-model-path="/nas/datasets/gen/ckpts/pretraining_ar_grid_grid_vox_v1_legs_indi_True_ctx_-1_gs_2_ngrids_250_vpred_1.0_scale_0_MotionDataset_processed_voxelized_64_bsz_1_max_verts_2000_max_permit_verts_2000/vertex_model/136800.pth" --sample-class-idx=0 #  --category-part-indicator=True # --debug=True   # --debug=True
