...
# grid size; quantization bits...
CUDA_VISIBLE_DEVICES=1 python trainer/pretraining_ar_vox.py  --exp-mode="pretraining"  --max-permit-vertices=900 --max-permit-faces=3000 --dataset-name="MotionDataset_processed_voxelized_64" --category="eyeglasses" --recenter-mesh=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=900 --max-faces=3000 --batch-size=1  --check-step=2000 --exp-flag="all_parts" --num-classes=120 --nn-vertices-predict-ratio=1.0  --not-remove-du=True --max-num-grids=900 --grid-size=1  --quantization-bits=6  --category-part-indicator=True --context-window=400  #  --category-part-indicator=True # --debug=True   # --debug=True #  --apply-random-scaling=1 
