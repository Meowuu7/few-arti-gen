...
# ### tabel, PolyGen Samples ####
CUDA_VISIBLE_DEVICES=6 python trainer/pretraining_xl_two_parts.py  --exp-mode="pretraining" --use-light-weight=True  --max-permit-vertices=400 --max-permit-faces=2000 --dataset-name="MotionDataset_processed_deci_part_first" --category="eyeglasses" --recenter-mesh=True --apply-random-shift=True --dataset-root-path='/nas/datasets/gen/datasets' --max-vertices=400 --max-faces=2000 --batch-size=10  --mask-low-ratio=0.30 --mask-high-ratio=0.50  --check-step=200 --exp-flag="all_parts"  # --debug=True
