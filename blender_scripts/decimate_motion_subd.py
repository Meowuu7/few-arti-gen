...
motion_dataset_root = "./data/MotionDataset_processed"
...
sv_subd_root_folder = os.path.join("~", "gen", "neuralSubdiv", "data_meshes", "{}-{}-valid".format(motion_cat, part_name))
...
for collapse_rt, subd_folder in zip(collapse_ratio, subd_folder_nm):
  ...
  for inst_nm in inst_nms:
    ...
    cur_inst_part_obj = os.path.join(cur_inst_folder, part_name + ".obj")
    if not os.path.exists(cur_inst_part_obj):
      print("{} not existed...".format(cur_inst_part_obj))
      continue
    imported_object = bpy.ops.import_scene.obj(filepath = cur_inst_part_obj)
    ...
    cur_sample_obj_sv_fn = os.path.join(cur_subd_sv_folder, "{:03d}".format(tot_n_samples + 1) + ".obj")
    tot_n_samples += 1
