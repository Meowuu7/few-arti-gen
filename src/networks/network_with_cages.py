...
from neuralcages_networks import NetworkFull, deform_with_MVC
...
class model(nn.Module):
    ...
    def forward5(self, src_pc, tar_pc, key_pts, dst_key_pts, w_pc,  src_verts, src_convex_pts, tar_verts, dst_convex_pts, glb_net=None):
        # src_pc = 
        ### src_pc, tar_pc;
        ### src_convex_pts: 
        ### src_convex_pts ---- only use convexes with the same idx in those two shapes ###
        ...
        outputs = self.cages(src_pc, tar_pc)
        def_pc = outputs["deformed"]
        
        return outputs 
    
    def forward6(self, src_pc, tar_pc, key_pts, dst_key_pts, w_pc,  src_verts, src_convex_pts, tar_verts, dst_convex_pts, glb_net=None):
        # src_pc = 
        ### src_pc, tar_pc;
        ### src_convex_pts: 
        ### src_convex_pts ---- only use convexes with the same idx in those two shapes ###
