...
class model(nn.Module):
    ...
    def forward(self, tot_src_pc, tot_tar_pc, tot_key_pts, tot_w_pc,  tot_w_mesh, tot_src_verts, tot_src_faces, tot_src_edges, tot_src_dofs, tot_tar_verts, tot_tar_edges, tot_tar_dofs):
        ...
        ''' connectivity loss '''
        joints_dir_pvp = self.joint_dir.unsqueeze(0), pivot_points[0].unsqueeze(0)
        dis_pc0_joint = model_utils.dis_pc_joint(offset_verts1, joints_dir_pvp) ### bsz
        dis_pc1_joint = model_utils.dis_pc_joint(def_verts[1], joints_dir_pvp) ### bsz
    ...
    def sample(self, tot_src_pc, tot_tar_pc, tot_key_pts, tot_w_pc,  tot_w_mesh, tot_src_verts, tot_src_faces, tot_src_edges, tot_src_dofs, tot_tar_verts, tot_tar_edges, tot_tar_dofs, optim):
        #### B, N, _
        #### network target driven ####
        B, N, _ = tot_src_pc[0].shape
        ...
        #   coefs = []
          for i_p, (src_pc, tar_pc, key_pts, w_pc, w_mesh, src_verts, src_edges, src_dofs, tar_verts, tar_edges, tar_dofs) in enumerate(zip(tot_src_pc, tot_tar_pc, tot_key_pts, tot_w_pc, tot_w_mesh, tot_src_verts, tot_src_edges, tot_src_dofs, tot_tar_verts, tot_tar_edges, tot_tar_dofs)):
            ...
            # coef = coef.requires_grad_(True)
            # basis = basis.requires_grad_(True)
            
            # bases.append(basis)
            # coefs.append(coef)
            
            key_pts = torch.zeros(key_pts.size(), dtype=torch.float32).cuda()
            key_pts[:, :, : ] = key_pts.data[:, :, :]
            # key_pts
            
            _, K, _ = key_pts.shape
            
            cur_basis = basis / torch.clamp(torch.norm(basis, p=2, dim=-1, keepdim=True), min=1e-6)
            
            print(f"i_p: {i_p}, i_step: {i_step}, coef: {coef}")
            
            def_key_pts = key_pts + torch.bmm(coef, cur_basis).view(B, K, 3)
        ...
        #   nex_bases = []
        #   nex_coefs = []
          for i_p, (cur_basis, cur_coef) in enumerate(zip(bases, coefs)):
            ...
            if grad_cur_basis is not None:
                ...
                delta_cur_basis = grad_cur_basis * 10
            ...
            bases[i_p] = nexx_basis.clone() # .requires_grad_(True)
            ...
            if grad_cur_coef is not None:
                ...
                delta_cur_coef = grad_cur_coef * 10 #  grad_cur_basis 
            ...
            coefs[i_p] = nex_coef.clone() # .requires_grad_(True)
            
        #   for cur_basis in nex_bases:
        #       print(f"cur_basis: {cur_basis.size()}")
        ...
        #   bases = [basis.requires_grad_(True) for basis in nex_bases]
        #   coefs = [coef.requires_grad_(True) for coef in nex_coefs]
        #   bases = nex_bases
        #   coefs = nex_coefs
