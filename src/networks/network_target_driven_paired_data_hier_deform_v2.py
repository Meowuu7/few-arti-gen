...
class model(nn.Module):
    def __init__(self, num_basis, opt=None):
        ...
        self.nn_codebook = 5
        self.codebook = nn.Embedding(num_embeddings=self.nn_codebook, embedding_dim=self.hidden_dim)
        torch.nn.init.xavier_uniform_(self.codebook.weight)
        
        ### select from codebooks; transform code books featuress for variational encoding ###
        self.codebook_tar_conv_trans_nets = nn.ModuleList()
        self.codebook_tar_conv_recon_trans_sets = nn.ModuleList()
        for i_c in range(self.nn_codebook):
          self.codebook_tar_conv_trans_nets.append(
            nn.Sequential(
                *[torch.nn.Conv1d(self.hidden_dim, 128, 1), 
                  # nn.BatchNorm1d(128), 
                  nn.ReLU(),
          ...
          self.codebook_tar_conv_recon_trans_sets.append(
            nn.Sequential(
    ...
    def select_from_codebook(self, src_out):
      # src_out: bsz x N_src x dim
      # codebook: n_codes x dim
      ## dists: bsz x N_src x n_codes x dim
      dists = torch.sum((src_out.unsqueeze(-2) - self.codebook.weight.unsqueeze(0).unsqueeze(0)) ** 2, dim=-1)
      minn_dists, minn_dists_idxes = torch.min(dists, dim=-1) ### bsz x N_src
      
      selected_codes = batched_index_select(self.codebook.weight, indices=minn_dists_idxes, dim=0) ### bsz x N_src x dim
      dists_selected_codes_feats = torch.sum((selected_codes - src_out) ** 2, dim=-1).mean() 
      ### 1) minn_dists as losses; 2) selected_codes_feats as code features
      return minn_dists.mean(), minn_dists_idxes, dists_selected_codes_feats
    
    def variational_tar_feature_encoding(self, minn_code_dists_idxes, tar_features, sampling=False):
      ### tar_features: (bsz x n_src) x dim x n
      ### minn_idxes: bsz x n_src; keypts transformation 
      bsz = minn_code_dists_idxes.size(0)
      # variational_tar_mus, variational_tar_log_sigmas = [], []
      variational_tar_recons = []
      
      tot_loss_prior = 0.0
      for i_bsz in range(bsz):
        cur_tar_features = tar_features[i_bsz].unsqueeze(0)
        cur_tar_minn_code_idx = int(minn_code_dists_idxes[i_bsz].item())
        cur_tar_trans_net = self.codebook_tar_conv_trans_nets[cur_tar_minn_code_idx]
        cur_tar_conv_recon_trans_net = self.codebook_tar_conv_recon_trans_sets[cur_tar_minn_code_idx]
        cur_tar_trans = cur_tar_trans_net(cur_tar_features)
        nn_tar_trans = cur_tar_trans.size(1)
        cur_tar_mu, cur_tar_log_sigma = cur_tar_trans[:, : nn_tar_trans // 2], cur_tar_trans[:, nn_tar_trans // 2: ]
        z = utils.reparameterize_gaussian(mean=cur_tar_mu, logvar=cur_tar_log_sigma)  # (B, F) ## global 
        
        log_pz_tar = utils.standard_normal_logprob(z).sum(dim=1) 
        ##### maximize the entropy of q_x(z); gaussian_entropy #####
        entropy_tar = utils.gaussian_entropy(logvar=cur_tar_log_sigma) ### 
        ...
        recon_tar_features = cur_tar_conv_recon_trans_net(z)
        recon_loss = torch.sum((cur_tar_features - recon_tar_features) ** 2, dim=-2).mean() 
        variational_tar_recons.append(recon_tar_features)
        loss_prior_tar += recon_loss
        tot_loss_prior += loss_prior_tar
      tot_loss_prior = tot_loss_prior / float(bsz)
      variational_tar_recons = torch.cat(variational_tar_recons, dim=0)
      return tot_loss_prior, variational_tar_recons #### prior loss and tar_recons 
    ...
    def forward(self, src_pc, tar_pc, key_pts, dst_key_pts, w_pc,  src_verts, src_faces, src_edges, src_dofs, tar_verts, tar_faces, tar_edges, tar_dofs, deform_net=None):
        ...
        if self.tar_basis > 0: # use target for deformation basis prediction ###
            ...
            code_minn_dists, code_idxes, _ = self.select_from_codebook(prob_net) ### code_idxes: bsz x K
            code_idxes = code_idxes.view(-1).contiguous()
            tot_loss_prior, variational_tar_recons = self.variational_tar_feature_encoding(code_idxes, selected_tar_keypts_out.contiguous().view(B * K, selected_tar_keypts_out.size(-2), selected_tar_keypts_out.size(-1)), sampling=False) 
            variational_tar_recons = variational_tar_recons.contiguous().view(B, K, -1, variational_tar_recons.size(-1))
            ...
            loss_prior_tar = tot_loss_prior + code_minn_dists
            
            
            selected_tar_keypts_out = variational_tar_recons
            loss_log_pz_tar = torch.zeros((1,), dtype=torch.float32).cuda().mean()
            loss_entropy_tar = torch.zeros((1,), dtype=torch.float32).cuda().mean()
    ...
    def sample(self, src_pc, tar_pc, key_pts, dst_key_pts, w_pc,  src_verts, src_faces, src_edges, src_dofs, tar_verts, tar_faces, tar_edges, tar_dofs, deform_net=None):
        ...
        if self.tar_basis > 0: # use target for deformation basis prediction ###
            ...
            code_minn_dists, code_idxes, _ = self.select_from_codebook(prob_net) ### code_idxes: bsz x K
            code_idxes = code_idxes.view(-1).contiguous()
            tot_loss_prior, variational_tar_recons = self.variational_tar_feature_encoding(code_idxes, selected_tar_keypts_out.contiguous().view(B * K, selected_tar_keypts_out.size(-2), selected_tar_keypts_out.size(-1)), sampling=True) 
            variational_tar_recons = variational_tar_recons.contiguous().view(B, K, -1, variational_tar_recons.size(-1))
