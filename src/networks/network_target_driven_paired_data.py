...
class model(nn.Module):
    ...
    def forward(self, src_pc, tar_pc, key_pts, dst_key_pts, w_pc,  src_verts, src_edges, src_dofs, tar_verts, tar_edges, tar_dofs):
        ...
        if self.tar_basis > 0: # use target for deformation basis prediction ###
            ...
            tar_keypts_out = self.select_keypts_features(dst_key_pts, tar_pc_downsampled, tar_out_trans) #### bsz x 
            ### selected_tar_keypts_out: bsz x K x neighbouring_k x feat_dim ###
            ### key_pts: bsz x n_keypts x 3; 
            selected_tar_keypts_out, neighbouring_tar_keypts_idxes = self.select_target_local_features(key_pts, dst_key_pts, tar_keypts_out, neighbouring_tar_keypts_idxes=None, neighbouring_tar_k=2)
        ...
        ###### 
        neighbouring_keypts_indicators = self.select_handles_via_radius(key_pts, radius=0.3, nearest_k=4)
