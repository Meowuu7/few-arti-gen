...
class model(nn.Module):
    ...
    def forward(self, batch):
        ...
        for i_p in range(len(def_pcs)):
          ...
          if self.bbox_assemble:
            ...
            cur_part_scaled_def_verts = cur_part_scaled_def_verts + cur_part_pos_offset.unsqueeze(1)
          ### maxx_y, 
          ## maxx_y = minn_y_of_first_part - y_extent / 2.
          ## maxx_x = maxx_x_of_first_part + x_extent / 2.
        ...
        keypts_1 = assembled_def_pcs[0][0] ## offset_keypts1[i_bsz]
        keypts_2 = assembled_def_pcs[1][0] ##j def_keyptses[1][i_bsz]
        
        
        
        
        keypts_1 = keypts_1.contiguous().view(keypts_1.size(0), 3)[tot_src_fps_idx[0]].contiguous().view(n_samples, 3).contiguous()
        keypts_2 = keypts_2.contiguous().view(keypts_2.size(0), 3)[tot_src_fps_idx[1]].contiguous().view(n_samples, 3).contiguous()
        ...
        if self.collision_stra == 0: ## loss
            ...
            syn_meshes_sequence, syn_meshes_sequence_ori, projected_bases, projected_coefs = self.sample(batch=batch, rt_basis_coefs=True, proj_for_training=True)
            # last_basis, last_coef = tot_bases[-1], tot_coefs[-1] ### last_basis: bsz x num_basis x (n_keypts x 3); last_coef: bsz x num_basis
            # projected_def_pcs = []
            projected_cd_losses = []
    ...
    def get_bases_coeffs_bak(self, tot_src_pc, tot_tar_pc, tot_key_pts, tot_w_pc,  tot_w_mesh, tot_src_verts, tot_src_faces, tot_src_edges, tot_src_dofs, tot_tar_verts, tot_tar_edges, tot_tar_dofs):
    ...
    def sample_bak(self, tot_src_pc, tot_tar_pc, tot_key_pts, tot_w_pc,  tot_w_mesh, tot_src_verts, tot_src_faces, tot_src_edges, tot_src_dofs, tot_tar_verts, tot_tar_edges, tot_tar_dofs):
        #### B, N, _
        #### network target driven ####
        B, N, _ = tot_src_pc[0].shape
    ...
    #### bases coeffs ####
    def get_bases_coeffs(self, batch, proj_for_training=False):
        ### get related quantities here ###
        tot_src_pc = batch['src_pc']
        tot_tar_pc = batch['tar_pc']
        tot_key_pts = batch['key_pts']
    ...
    def sample(self, batch, rt_basis_coefs=False, proj_for_training=False):
        ### get related quantities here ###
        ### get 
        tot_src_pc = batch['src_pc']
        tot_tar_pc = batch['tar_pc']
        ...
        tot_bases, tot_coefs, tot_src_fps_idx = self.get_bases_coeffs(batch, proj_for_training=proj_for_training)
        ...
        for i_step in range(n_steps):
            ...
            #### collision_loss_func --> for th collision loss ####
            # n_sim_steps = 100
            n_sim_steps = self.opt.train_n_sim_steps if proj_for_training else self.opt.n_sim_steps
    ...
    ### pc: bsz x N x 3; joint: 
            ...
            for i_p, (cur_basis, cur_coef) in enumerate(zip(tot_bases, tot_coefs)):
                ...
                [grad_cur_basis, grad_cur_coef] = autograd.grad(
                    outputs=collision_loss,
                    inputs=[cur_basis, cur_coef],
                ...
                if grad_cur_coef is not None:
                    ...
                    # print(f"delta_cur_coef: {delta_cur_coef}")
                    # print(f"cur_coef: {cur_coef}")
                    nex_coef = cur_coef + delta_cur_coef
                    # print(f"nex_coef: {nex_coef}")
                else:
                ...
                tot_coefs[i_p] = nex_coef.clone() # .requires_grad_(True)
