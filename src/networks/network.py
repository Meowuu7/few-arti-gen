...
class model(nn.Module):
    ...
    def forward(self, src_pc, tar_pc, key_pts, w_pc):
        ...
        #### target global ####
        tar_global = self.pointnet(tar_pc, True) ### target pc ##3 flow predition and composition? 
        ...
        w_pc1 = w_pc.transpose(2, 1).unsqueeze(2)  # B K 1 N ## w_pc: b x n x k --> b x k x n --> b x k x 1 x n
        ...
        # coef_range = coef_range.view(B, self.num_basis, 2) * 0.5 # coef_multiplier
        coef_range = coef_range.view(B, self.num_basis, 2) * self.coef_multiplier
        ...
        for i_s in range(10):
            ...
            tot_sampled_def_keypts.append(sample_def_key_pts)
        ...
        if self.opt is not None:
            return def_key_pts, def_pc, cd_loss, basis, coef, sample_def_key_pts, sym_loss, coef_range, tot_sampled_def_keypts
        else:
