...
#     # weight_flag = None if (opt.hier_stage == 0 or (not with_cat_deform)) else "cat_deform_net"
#     # weight_flag = "cat_deform_net" if (opt.hier_stage == 1 and opt.with_cat_deform) else None
#     # print(f"Loading net from {opt.net_path}")
...
#     safe_load_ckpt_common(glb_net, net_state_dict) ### laod net path ###
    ...
    # if opt.hier_stage == 1:
    #     ori_deform_dict = net.cat_deform_net.state_dict()
    #     safe_load_ckpt_common(deform_net, ori_deform_dict, weight_flag=None)
    #     deform_net.eval()
...
for epoch in range(start_epoch, opt.epoch):
    ...
    #     cur_part_mesh_idx = None
    #     for i_p in range(opt.n_parts):
    #         cur_part_dset_train = part_dataset_train[i_p] ### dataset_train
    #         cur_part_rt_dict = cur_part_dset_train.__getitem__(ori_data_idx, cur_data_idx)
    #         ### part_networks ###
    #         cur_part_network = part_networks[i_p] ### part neworks 
    ...
    #         ### key_pts ###
    #         rt_dict = cur_part_network.forward7(src_pc, tar_pc, key_pts, dst_key_pts, w_pc, src_verts, src_cvx_to_pts , tar_verts, dst_cvx_to_pts)
    #         mesh_rt_dict = cur_part_network.forward7(src_verts[0].unsqueeze(0), tar_verts[0].unsqueeze(0), key_pts, dst_key_pts, w_pc,  src_verts,src_cvx_to_pts , tar_verts, dst_cvx_to_pts)
    ...
    #         tot_cvx_pts_idxes = mesh_rt_dict["tot_cvx_pts_idxes"]
    #         def_part_pc = get_def_pcs_by_cages(tot_bsz_cages_ori, part_pc, tot_sorted_basis, tot_bsz_coefs, tot_cvx_pts_idxes) 
