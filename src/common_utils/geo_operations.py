...
from .operations import normalize, scatter_add, cross_product_2D, gather_points, dot_product
import numpy as np
...
def mean_value_coordinates(points, polygon, verbose=False):
    ...
    # Ai = cross_product_2D(si[:, :2, ...], sip[:, :2, ...], dim=1)/2
    # Ai = cross_product_3D(si, sip, dim=1)/2
    # Ai = torch.cross(si, sip, dim=1).mean(dim=1) # / 2.
