...
def jsd_between_point_cloud_sets(
        sample_pcs, ref_pcs, resolution=56):
    ...
    sample_grid_var = entropy_of_occupancy_grid(
        sample_pcs, resolution, in_unit_sphere, verbose=True)[1]
    ref_grid_var = entropy_of_occupancy_grid(
        ref_pcs, resolution, in_unit_sphere, verbose=True)[1]
...
def entropy_of_occupancy_grid(
    ...
    if abs(np.max(pclouds)) > bound or abs(np.min(pclouds)) > bound:
        ...
        if verbose:
            warnings.warn(f'Point-clouds are not in unit cube, max: {np.max(pclouds)}, min: {np.min(pclouds)}')
