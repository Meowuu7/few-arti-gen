...
def load_joint_infos_partnet_mobility(motion_attrs):
  ...
  for k in motion_attrs:
    ...
    cur_part_motion_type = motion_attrs[k]["motion_type"] ## revolute or prismatic
    cur_part_axis = motion_attrs[k]['axis']
    if 'origin' in cur_part_axis:    
      cur_part_joint_center = cur_part_axis['origin']
      cur_part_joint_dir = cur_part_axis['direction']
...
class ChairDataset(torch.utils.data.Dataset):
    def __init__(self, phase="train", data_dir="../data/chair", opt=None):
        ...
        print(f"Object data root folder: {self.obj_data_root_folder}.") ### 
        ...
        self.tot_obj_inst_folders = sorted(self.tot_obj_inst_folders)
        
        self.tot_obj_inst_folders = [self.tot_obj_inst_folders[idxx] for idxx in tot_insts]
    ...
    def load_data_from_model_indicator(self, rt_folder, model_indicator):
        ...
        cur_cvx_to_verts_fn = cur_model + f"_manifold_cvx_to_verts.npy"
    ...
    # }
        ...
        for i_p, (idxx, cur_src_n_models, cur_dst_n_models) in enumerate(zip(src_idxes, self.src_n_models, self.dst_n_models)):
          ...
          if self.src_pc[i_p][src_idx] is None:
            src_pc, src_key_pts, src_vertices, src_faces, src_w_pc, src_w_mesh, src_cvx_to_pts = self.load_data_from_model_indicator(self.src_folder[i_p], self.src_models[i_p][src_idx])
          ...
          if self.dst_pc[i_p][dst_idx] is None:
            tar_pc, dst_key_pts, dst_vertices, dst_faces, dst_w_pc, dst_w_mesh, dst_cvx_to_pts = self.load_data_from_model_indicator(self.dst_folder[i_p], self.dst_models[i_p][dst_idx])
          ...
          tar_pc, dst_key_pts, dst_vertices, dst_faces, dst_w_pc, dst_w_mesh, dst_cvx_to_pts = self.dst_pc[i_p][dst_idx], self.dst_key_pts[i_p][dst_idx], self.dst_mesh_vertices[i_p][dst_idx], self.dst_mesh_faces[i_p][dst_idx], self.dst_w_pc[i_p][dst_idx], self.dst_w_mesh[i_p][dst_idx], self.dst_cvx_to_pts[i_p][src_idx]
