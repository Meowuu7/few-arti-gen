...
def normalize(tsr, dim):
  # tsr = tsr / torch.norm(tsr, dim=dim, keepdim=True)
  tsr = tsr / torch.clamp(torch.norm(tsr, dim=dim, keepdim=True), min=1e-5)
  return tsr

def _scatter_add(src, idx, dim, out_size, fill):
...
def mean_value_coordinates_3D(query, vertices, faces, verbose=False):
    ...
    # assert(check_values(theta_i)) #### remvoe assert 
    # B,P,F,1
